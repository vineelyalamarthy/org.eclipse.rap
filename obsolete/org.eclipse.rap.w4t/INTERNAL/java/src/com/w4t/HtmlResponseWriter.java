...
public class HtmlResponseWriter extends Writer {
 ...
 /**
   ...
   * {@link IResourceManager#register(String, String, com.w4t.IResourceManager.RegisterOptions)
    ...
    throws IOException
  {
    ...
    if( value == null ) {
      doWrite( " " );
      doWrite( name );
      if( W4TContext.getBrowser().isXHTMLCapable() ) {
        doWrite( "=\"" );
        doWrite( name );
        doWrite( "\"" );        
      }
    ...
    throws IOException
  {
    ...
    if( avoidEscape ) {
      ...
      doWrite( RenderUtil.encodeHTMLEntities( text.toString() ) );
    }
  }
  ...
  private static String createClassName( final String key ) {
    return Style.CLASS_PREFIX + Integer.toHexString( key.hashCode() );
  }
