...
/**
 * Contains static methods for performing simple geometric operations on the RWT
 * geometry classes.
 * 
 * @since 3.0
 */
public class Geometry {
   ...
   * <p>
   * This is preferred over the real distance when searching for the closest
   * point, since it avoids square roots.
   * </p>
   * 
  ...
  /**
   * Returns the square of the magnitude of the given 2-space vector
   * (represented using a point)
   * 
  ...
  /**
   * Returns a new point whose coordinates are the minimum of the coordinates of
   * the given points
   * 
   * @param p1 a Point
   * @param p2 a Point
   * @return a new point whose coordinates are the minimum of the coordinates of
   *         the given points
   * @since 3.0
   */
  ...
  /**
   * Returns a new point whose coordinates are the maximum of the coordinates of
   * the given points
   * 
   * @param p1 a Point
   * @param p2 a Point
   * @return point a new point whose coordinates are the maximum of the
   *         coordinates
   * @since 3.0
   */
   ...
   * Directions are given using RWT direction constants, and the resulting
   * vector is in the screen's coordinate system. That is, the vector (0, 1) is
   * down and the vector (1, 0) is right.
   * 
   * @param distance magnitude of the vector
   * @param direction one of RWT.TOP, RWT.BOTTOM, RWT.LEFT, or RWT.RIGHT
   * @return a point representing a vector in the given direction with the given
   *         magnitude
   * @since 3.0
   */
  ...
  /**
   * Swaps the X and Y coordinates of the given rectangle, along with the height
   * and width.
   * 
   ...
   * @param width if true, the width is modified. If false, the height is
   *          modified.
   ...
   * 
   * @param toSet rectangle to modify
   * @param width if true, the x coordinate is modified. If false, the y
  ...
  /**
   * Returns the distance of the given point from a particular side of the given
   * rectangle. Returns negative values for points outside the rectangle.
   * 
  ...
  /**
   * Extrudes the given edge inward by the given distance. That is, if one side
   * of the rectangle was sliced off with a given thickness, this returns the
   ...
   *          (that is, the resulting rectangle will overlap the original iff
   *          this is positive).
   * @param orientation the side to extrude. One of RWT.LEFT, RWT.RIGHT,
   *          RWT.TOP, or RWT.BOTTOM. The resulting rectangle will always share
  ...
  public static Rectangle getExtrudedEdge( Rectangle toExtrude,
                                           ...
                                           int orientation )
  {
    Rectangle bounds = new Rectangle( toExtrude.x,
                                      toExtrude.y,
                                      toExtrude.width,
                                      toExtrude.height );
  ...
  /**
   * Returns the opposite of the given direction. That is, returns RWT.LEFT if
   * given RWT.RIGHT and visa-versa.
   * 
   * @param rwtDirectionConstant one of RWT.LEFT, RWT.RIGHT, RWT.TOP, or
   *          RWT.BOTTOM
   * @return one of RWT.LEFT, RWT.RIGHT, RWT.TOP, or RWT.BOTTOM
   * @since 3.0
   */
  public static int getOppositeSide( int rwtDirectionConstant ) {
    switch( rwtDirectionConstant ) {
      case RWT.TOP:
        return RWT.BOTTOM;
      case RWT.BOTTOM:
        return RWT.TOP;
    ...
    return rwtDirectionConstant;
  }

  /**
   * Converts the given boolean into an RWT orientation constant.
   * 
   * @param horizontal if true, returns RWT.HORIZONTAL. If false, returns
   *          RWT.VERTICAL
   * @return RWT.HORIZONTAL or RWT.VERTICAL.
   * @since 3.0
   */
  public static int getRwtHorizontalOrVerticalConstant( boolean horizontal ) {
    if( horizontal ) {
      return RWT.HORIZONTAL;
    }
    return RWT.VERTICAL;
  }

  /**
   * Returns true iff the given RWT side constant corresponds to a horizontal
   * side of a rectangle. That is, returns true for the top and bottom but false
   * for the left and right.
   * 
   * @param rwtSideConstant one of SWT.TOP, SWT.BOTTOM, SWT.LEFT, or SWT.RIGHT
   * @return true iff the given side is horizontal.
   * @since 3.0
   */
  public static boolean isHorizontal( int rwtSideConstant ) {
    return !( rwtSideConstant == RWT.LEFT || rwtSideConstant == RWT.RIGHT );
  }
   ...
   * Negative values cause the rectangle to contract. Does not allow the
   * rectangle's width or height to be reduced below zero.
   * 
   * @param rect normalized rectangle to modify
   * @param left distance to move the left edge outward (negative values move
   *          the edge inward)
   * @param right distance to move the right edge outward (negative values move
   *          the edge inward)
   * @param top distance to move the top edge outward (negative values move the
   *          edge inward)
   * @param bottom distance to move the bottom edge outward (negative values
   *          move the edge inward)
   * @since 3.1
   */
  ...
  /**
   * Normalizes the given rectangle. That is, any rectangle with negative width
   * or height becomes a rectangle with positive width or height that extends to
   * the upper-left of the original rectangle.
   * 
  ...
  /**
   * Converts the given rectangle from display coordinates to the local
   * coordinate system of the given object into display coordinates.
   * 
  ...
  public static Rectangle toControl( Control coordinateSystem,
                                     Rectangle toConvert )
  {
    return ( coordinateSystem.getDisplay().map( null,
                                                coordinateSystem,
                                                toConvert ) );
  }

  /**
   * Converts the given rectangle from the local coordinate system of the given
   * object into display coordinates.
   * 
  ...
  public static Rectangle toDisplay( Control coordinateSystem,
                                     Rectangle toConvert )
  {
    return ( coordinateSystem.getDisplay().map( coordinateSystem,
                                                null,
                                                toConvert ) );
  }
   ...
   * Returns a combination of RWT.LEFT, RWT.RIGHT, RWT.TOP, and RWT.BOTTOM,
   * combined with bitwise or (for example, returns RWT.TOP | RWT.LEFT if the
   * point is to the upper-left of the rectangle). Returns 0 if the point lies
   * within the rectangle. Positions are in screen coordinates (ie: a point is
   * to the upper-left of the rectangle if its x and y coordinates are smaller
   * than any point in the rectangle)
   * 
   * @param boundary normalized boundary rectangle
   * @param toTest point whose relative position to the rectangle is being
   *          computed
   * @return one of RWT.LEFT | RWT.TOP, RWT.TOP, RWT.RIGHT | RWT.TOP, RWT.LEFT,
   *         0, RWT.RIGHT, RWT.LEFT | RWT.BOTTOM, RWT.BOTTOM, RWT.RIGHT |
   *         RWT.BOTTOM
   * @since 3.0
   */
   ...
   *         rectangle. Returns positive values for points inside the rectangle
   *         and negative values for points outside the rectangle.
   * @since 3.1
   */
  ...
  public static int getClosestSide( Rectangle boundary, Point toTest ) {
    int[] sides = new int[]{
      RWT.LEFT, RWT.RIGHT, RWT.TOP, RWT.BOTTOM
    };
    int closestSide = RWT.LEFT;
  ...
  /**
   * Sets the x,y position of the given rectangle. For a normalized rectangle (a
   * rectangle with positive width and height), this will be the upper-left
   ...
   * (rectangles with positive width and height), this is the upper-left corner
   * of the rectangle.
   * 
  ...
  /**
   * Repositions the 'inner' rectangle to lie completely within the bounds of
   * the 'outer' rectangle if possible. One use for this is to ensure that, when
   * setting a control's bounds, that they will always lie within its parent's
   * client area (to avoid clipping).
   * 
   * @param inner The 'inner' rectangle to be repositioned (should be smaller
   *          than the 'outer' rectangle)
