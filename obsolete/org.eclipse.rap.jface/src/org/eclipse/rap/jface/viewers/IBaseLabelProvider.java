 ...
 * does not actually define the methods to get the label(s) for an element. This
 * interface should never be directly implemented. Most viewers will take either
 * an <code>ILabelProvider</code> or an <code>ITableLabelProvider</code>.
 * <p>
 ...
 * </p>
 * <p>
 * Label providers can be used outside the context of viewers wherever images
 * are needed. When label providers are used in this fashion it is the
 * responsibility of the user to ensure <code>dispose</code> is called when
...
public interface IBaseLabelProvider {
   ...
   * non-structural viewer update. If the property mentioned in the update does
   * not affect the label, then the viewer need not update the label.
   * 
