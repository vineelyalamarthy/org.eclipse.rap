...
public abstract class ContentViewer extends Viewer {
  ...
  /**
   * This viewer's label provider. Initially <code>null</code>, but lazily
   * initialized (to a <code>SimpleLabelProvider</code>).
   */
  private IBaseLabelProvider labelProvider = null;
  /**
   * This viewer's label provider listener. Note: Having a viewer register a
   * label provider listener with a label provider avoids having to define
   * public methods for internal events.
   */
  ...
  /**
   * Creates a content viewer with no input, no content provider, and a default
   * label provider.
   */
  ...
  /**
   * The <code>ContentViewer</code> implementation of this
   * <code>IInputProvider</code> method returns the current input of this
   * viewer, or <code>null</code> if none. The viewer's input provides the
   * "model" for the viewer's content.
   */
   ...
   * <p>
   * The <code>ContentViewer</code> implementation of this method returns the
   * label provider recorded in an internal state variable; if none has been set
   * (with <code>setLabelProvider</code>) a <code>SimpleLabelProvider</code>
   * will be created, remembered, and returned. Overriding this method is
   ...
   * <p>
   * The <code>ContentViewer</code> implementation of this method disposes of
   * this viewer's label provider and content provider (if it has one).
   * Subclasses should override this method to perform any additional cleanup of
   * resources; however, overriding methods must invoke
   ...
   * <p>
   * All subclasses must call this method when their control is first
   * established.
   * </p>
   * <p>
   * The <code>ContentViewer</code> implementation of this method hooks
   * dispose events for the given control. Subclasses may override if they need
   * to add other control hooks; however, <code>super.hookControl</code> must
   * be invoked.
   * </p>
   * 
   ...
   * <p>
   * The <code>ContentViewer</code> implementation of this method ensures that
   * the given label provider is connected to this viewer and the former label
   * provider is disconnected from this viewer. Overriding this method is
   * generally not required; however, if overriding in a subclass,
  ...
  public void setLabelProvider( IBaseLabelProvider labelProvider ) {
    ...
    refresh();
    // Dispose old provider after refresh, so that items never refer to stale
    // images.
    if( oldProvider != null ) {
      oldProvider.dispose();
    }
