...
public interface ITreePathContentProvider extends IStructuredContentProvider {
   ...
   * Implementors may want to use the additional context of the complete path of
   * a parent element in order to decide which children to return.
   * <p>
   * The provided path is relative to the input. The root elements must be
   * obtained by calling {@link IStructuredContentProvider#getElements(Object)}.
   * </p>
  ...
  /**
   * Return the possible parent paths for the given element. An empty array can
   * be returned if the paths cannot be computed. If the element is a potential
   * child of the input of the viewer, an empty tree path should be an entry in
   * the returned array.
   * 
