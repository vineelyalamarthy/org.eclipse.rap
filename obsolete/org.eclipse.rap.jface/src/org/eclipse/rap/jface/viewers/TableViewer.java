...
public class TableViewer extends StructuredViewer {
  ...
  // TODO: virtual tables not yet supported
  ...
  /**
   * TableColorAndFontNoOp is an optimization for tables without color and font
   * support.
   * 
  ...
  private class TableColorAndFontNoOp {
    ...
    /**
     * Set the fonts and colors for the tableItem if there is a color and font
     * provider available.
     * 
     ...
     */
    public void setFontsAndColors( TableItem tableItem,
                                   Object element,
                                   int column )
    {
    }
  ...
  /**
   * TableColorAndFontCollector is an helper class for color and font support
   * for tables that support the ITableFontProvider and the ITableColorProvider.
   * 
   * @see ITableColorProvider
   * @see ITableFontProvider
   */
  private class TableColorAndFontCollector extends TableColorAndFontNoOp {
    ...
    /**
     * Create an instance of the receiver. Set the color and font providers if
     * provider can be cast to the correct type.
     * 
                                   ...
                                   int column )
    {
      if( colorProvider != null ) {
        // TODO: table item doesn't allow setting the font
  ...
  /**
   * Initialize the virtual manager to manage the virtual state if the table is
   * VIRTUAL. If not use the default no-op version.
   * 
   * @param style
   */
  ...
  // }
  /**
   * Adds the given elements to this table viewer. If this viewer does not have
  ...
  /**
   * Adds the given element to this table viewer. If this viewer does not have a
   * sorter, the element is added at the end; otherwise the element is inserted
   ...
   * correspond with the columns of the table control. They are used to identify
   * the column in a cell modifier.
   * 
   ...
   * method returns the label provider, which in the case of table viewers will
   * be an instance of either <code>ITableLabelProvider</code> or
  ...
  protected void inputChanged( Object input, Object oldInput ) {
    // TODO: setRedraw not supported yet
  ...
  /**
   * Inserts the given element into this table viewer at the given position. If
  ...
  protected void internalRefresh( Object element, boolean updateLabels ) {
    ...
    if( element == null || equals( element, getRoot() ) ) {
      // eihter no element or root element -> refresh all
      // TODO: virtual tables not yet supported
      // if(virtualManager == null) {
  ...
  // }
  /**
   * Refresh all of the elements of the table. update the labels if updatLabels
   * is true;
   * 
   * @param updateLabels
   * @since 3.1
  ...
  private void internalRefreshAll( boolean updateLabels ) {
    ...
    for( int i = 0; i < min; ++i ) {
      ...
      if( equals( children[ i ], item.getData() ) ) {
        ...
        // TODO: TableItem currently does not support more complex setting of
        // images
  ...
  public boolean isCellEditorActive() {
    // TODO: Editing table data not yet supported, remove replacement code
    return false;
    // return tableViewerImpl.isCellEditorActive();
  }

  /**
   * Removes the given elements from this table viewer. The selection is updated
   * if required.
   * <p>
   * This method should be called (by the content provider) when elements have
  ...
  /**
   * Removes the given element from this table viewer. The selection is updated
   * if necessary.
   * <p>
   * This method should be called (by the content provider) when a single
   ...
   * <p>
   * If the label provider is an {@link ITableLabelProvider}, then it provides
   * a separate label text and image for each column. Implementers of
   ...
   * ILazyContentProvider as lookup is done by indices rather than elements and
   * may require population of the entire table in worse case.
   * </p>
   * <p>
   * Use Table#setSelection(int[] indices) and Table#showSelection() if you wish
   * to set selection more effeciently when using a ILazyContentProvider.
   * </p>
   * 
  ...
  // for (int i = 0; virtualElements.size() > 0 && i <
  // getTable().getItemCount(); i++) {
  ...
  // else{
  //      
  // if(count != list.size()){//As this is expensive skip it if all have been
  // found
  ...
  protected Object[] getRawChildren( Object parent ) {
    // TODO: virtual tables not yet supported
    // Assert.isTrue(!(getContentProvider() instanceof
    // ILazyContentProvider),"Cannot get raw children with an
    // ILazyContentProvider");//$NON-NLS-1$
