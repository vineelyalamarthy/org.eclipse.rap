...
public abstract class Window implements IShellProvider {
  ...
  /**
   * An array of images to be used for the window. It is expected that the array
   * will contain the same icon rendered at different resolutions.
   */
  ...
  /**
   * The default orientation of the window. By default it is SWT#NONE but it can
   * also be SWT#LEFT_TO_RIGHT or SWT#RIGHT_TO_LEFT
   */
  private static int orientation = RWT.NONE;
  /**
  ...
  private static IShellProvider defaultModalParent = new IShellProvider() {
    ...
    public Shell getShell() {
      ...
      // Make sure we don't pick a parent that has a modal child (this can lock
      // the app)
      if( parent == null ) {
        // If this is a top-level window, then there must not be any open modal
        // windows.
...
//  private FontChangeListener fontChangeListener;
   ...
   * parent shell. Note that the window will have no visual representation until
   * it is told to open. By default, <code>open</code> does not block.
   * 
   * @param parentShell the parent shell, or <code>null</code> to create a
   *          top-level shell. Try passing "(Shell)null" to this method instead
   *          of "null" if your compiler complains about an ambiguity error.
   * @see #setBlockOnOpen
  ...
  /**
   * Creates a new window which will create its shell as a child of whatever the
   * given shellProvider returns.
   * 
   * @param shellProvider object that will return the current parent shell. Not
   *          null.
   */
   ...
   * <p>
   * The default implementation of this framework method sets the shell's image
   * and gives it a grid layout. Subclasses may extend or reimplement.
   * </p>
   * 
  ...
  /**
   * Creates the layout for the shell. The layout created here will be attached
   * to the composite passed into createContents. The default implementation
   * returns a GridLayout with no margins. Subclasses that change the layout
   * type by overriding this method should also override createContents.
   * <p>
   * A return value of null indicates that no layout should be attached to the
   * composite. In this case, the layout may be attached within createContents.
   * </p>
   * 
   ...
   * <p>
   * It is common practise to create and return a single composite that contains
   * the entire window contents.
   * </p>
   * <p>
   * The default implementation of this framework method creates an instance of
   * <code>Composite</code>. Subclasses may override.
   * </p>
   * 
   * @param parent the parent composite for the controls in this window. The
   ...
   * <p>
   * The default implementation of this framework method creates a new shell and
   * configures it using <code/>configureShell</code>. Rather than override
   * this method, subclasses should instead override <code/>configureShell</code>.
   * </p>
   * 
   * @return the shell
   */
  protected final Shell createShell() {
    ...
    result.addControlListener( resizeListener );
    result.setData( this );
    configureShell( result );
    result.setData( this );
    // Add shell listener
    result.addShellListener( getShellListener() );
    // Set the layout
    configureShell( result );
  ...
  /**
   * Returns the default image. This is the image that will be used for windows
   * that have no shell image at the time they are opened. There is no default
   * image unless one is installed via <code>setDefaultImage</code>.
   * 
  ...
  /**
   * Returns the array of default images to use for newly opened windows. It is
   ...
   * implementation centers the shell horizontally (1/2 of the difference to the
...
//     Rectangle monitorBounds = new Rectangle( 0, 0, 1024, 768 );
   ...
   */
  private static Shell getModalChild( final Shell[] toSearch ) {
    // int modal = RWT.APPLICATION_MODAL | RWT.SYSTEM_MODAL | RWT.PRIMARY_MODAL;
    int modal = RWT.APPLICATION_MODAL;
  ...
  protected Shell getParentShell() {
    ...
    if( ( getShellStyle() & modal ) != 0 ) {
      // If this is a modal shell with no parent, pick a shell using
      // defaultModalParent.
      if( parent == null ) {
   ...
   * 
   * @return this window's shell, or <code>null</code> if this window's shell
   ...
   * window's shell.
   * <p>
   * The default implementation of this framework method returns a new listener
   * that makes this window the active window for its window manager (if it has
   * one) when the shell is activated, and calls the framework method
   * <code>handleShellCloseEvent</code> when the shell is closed. Subclasses
  ...
  protected ShellListener getShellListener() {
    return new ShellAdapter() {
      ...
      public void shellClosed( ShellEvent event ) {
        // TODO [rst] implement doit flag on Shell event
   ...
   * Subclassers should call <code>setShellStyle</code> to change this value,
...
//  }
   ...
   * <code>close</code>. Subclasses may reimplement.
   * </p>
   * <p>
   * <strong>Important:</strong> In contrast to SWT, subclasses should
   * overwrite but not extend this method. Subclasses that extend this method
   * usually call
   * <code>setReturnCode</code> <em>after</em> <code>super.handleShellCloseEvent</code>.
   * In RWT, a return code that is set after closing the shell will not be
   * passed to the {@link IWindowCallback}.
   * </p>
   */
   ...
   * shell have not already been modified. Subclasses may extend or reimplement.
   * </p>
   */
  protected void initializeBounds() {
    ...
    if( !resizeHasOccurred ) { // Check if shell size has been set already.
      ...
      shell.setBounds( getConstrainedShellBounds( rect ) );
    }
  }
...
//   * closes.
//   * 
//   * @param shouldBlock <code>true</code> if the <code>open</code> method
//   *          should not return until the window closes, and <code>false</code>
...
//  }
   ...
   */
  public static void setDefaultImage( final Image image ) {
   ...
   * 
   * @see org.eclipse.swt.widgets.Decorations#setImages(org.eclipse.swt.graphics.Image[])
   * @param images the array of images to be used when this window is opened
   */
  public static void setDefaultImages( final Image[] images ) {
   ...
   * 
   * @param newParentShell The new parent shell; this value may be
   *          <code>null</code> if there is to be no parent.
   */
   ...
   * by <code>open</code> if block on open is enabled. For non-blocking opens,
   * the return code needs to be retrieved manually using
...
//  }
   ...
   * initial window position, and subclasses can use this as a utility method if
   * they want to limit the region in which the window may be moved.
   * 
...
//     Rectangle bounds = mon.getClientArea();
   ...
   */
  public void setWindowManager( final WindowManager manager ) {
   ...
   */
  public static void setExceptionHandler( final IExceptionHandler handler ) {
  ...
  /**
   * Sets the default parent for modal Windows. This will be used to locate the
   * parent for any modal Window constructed with a null parent.
   * 
   ...
   */
  public static void setDefaultModalParent( final IShellProvider provider ) {
    defaultModalParent = provider;
  }

  /**
   * Gets the default orientation for windows. If it is not set the default
   * value will be unspecified (SWT#NONE).
   * 
   ...
   * 
   * @param defaultOrientation one of SWT#RIGHT_TO_LEFT, SWT#LEFT_TO_RIGHT
   *          ,SWT#NONE
   * @see SWT#RIGHT_TO_LEFT
   * @see SWT#LEFT_TO_RIGHT
   * @see SWT#NONE
   */
  public static void setDefaultOrientation( final int defaultOrientation ) {
