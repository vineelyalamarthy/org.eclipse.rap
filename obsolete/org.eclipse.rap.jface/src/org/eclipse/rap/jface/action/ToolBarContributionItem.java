 ...
 * IBM Corporation - initial API and implementation Kostadin Bajalcaliev
 * <kbajalc@netcetera.com.mk> - Fix for bug 145378 - [Coolbar]RCP appication
 * loses Icons/actions with IDE Stable Builds RC6 and RC7
 ******************************************************************************/
 ...
 * <code>ContributionItem</code> but and provides some additional methods to
  ...
  implements IToolBarContributionItem
{
  ...
  /**
   * A constant used by <code>setMinimumItemsToShow</code> and
   * <code>getMinimumItemsToShow</code> to indicate that all tool items should
   * be shown in the cool item.
   */
  public static final int SHOW_ALL_ITEMS = -1;
  /**
   * The pull down menu used to list all hidden tool items if the current size
   * is less than the preffered size.
   */
  ...
  /**
   * Checks whether this contribution item has been disposed. If it has, and the
   * tracing options are active, then it prints some debugging information.
   * 
...
//      }
  ...
  public void dispose() {
    ...
    /*
     * We need to dispose the cool item or we might be left holding a cool item
     * with a disposed control.
     */
...
//        coolItem = new CoolItem( coolBar, flags, index );
      ...
      // Handle Context Menu
      // ToolBarManager.createControl can actually return a pre-existing
      // control.
...
//        } );
   ...
   * 
   * @return <code>true</code> if chevron support is enabled,
...
//  }
  ...
  /**
   * A contribution item is visible iff its internal state is visible
   * <em>or</em> the tool bar manager contains something other than group
...
//    }
  ...
  /**
   * Sets the current height of the cool item. Update(SIZE) should be called to
   * adjust the widget.
   * 
  ...
  /**
   * Sets the current width of the cool item. Update(SIZE) should be called to
   * adjust the widget.
   * 
  ...
  /**
   * Enables or disables chevron support for the cool item. By default, chevron
   * support is enabled.
   * 
   * @param value <code>true</code> to enable chevron support,
   *          <code>false</code> otherwise.
   */
   ...
   * 
   * @param changeCurrentSize <code>true</code> if the current size should be
   *          changed to the preferred size, <code>false</code> to not change
   *          the current size
   */
  private void updateSize( boolean changeCurrentSize ) {
    ...
    try {
          ...
          || ( toolBar.getItemCount() <= 0 ) )
      {
        ...
        // item
//        Point toolBarSize = toolBar.computeSize( RWT.DEFAULT, RWT.DEFAULT );
        Point toolBarSize
          = toolBar.computeSize( RWT.DEFAULT, RWT.DEFAULT, true );
        // TODO: [fappel] remove this size hack...
