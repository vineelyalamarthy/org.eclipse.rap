...
// import org.eclipse.rap.rwt.widgets.Decorations;
...
public class MenuManager extends ContributionManager implements IMenuManager {
  ...
  /**
   * The menu item widget; <code>null</code> before creation and after
   * disposal. This field is used when this menu manager is a sub-menu.
   */
   ...
   * <code>null</code>. Typically used for creating a sub-menu, where it
   * doesn't need to be referred to by id.
   * 
  ...
  /**
   * Creates and returns an SWT menu bar control for this menu, for use in the
   * given <code>Shell</code>, and installs all registered contributions.
   * Does not create a new control if one already exists. This implementation
   * simply calls the <code>createMenuBar(Decorations)</code> method
   * 
   * @param parent the parent decorations
   * @return the menu control
   * @XXXdeprecated use <code>createMenuBar(Decorations)</code> instead.
   */
  public Menu createMenuBar( Shell parent ) {
    // TODO: [fappel] currently we don't have Decorations so we use this...
    if( !menuExist() ) {
      menu = new Menu( parent, RWT.BAR );
      update( false );
    }
    return menu;
  ...
  public void fill( Menu parent, int index ) {
    if( menuItem == null || menuItem.isDisposed() ) {
      if( index >= 0 ) {
        // TODO: [fappel] missing constructor
        // menuItem = new MenuItem(parent, RWT.CASCADE, index);
        menuItem = new MenuItem( parent, RWT.CASCADE );
      } else {
        menuItem = new MenuItem( parent, RWT.CASCADE );
      }
  ...
  public boolean isVisible() {
    ...
    // menus arent visible if all of its children are invisible (or only
   ...
   * @param recursive <code>true</code> means recursively update all submenus,
   *          and <code>false</code> means just this menu
   */
  ...
  private void updateMenuItem() {
     ...
     * sub-menu is found. See bug 30833 for more details. if (menuItem != null &&
     * !menuItem.isDisposed() && menuExist()) { IContributionItem items[] =
     * getItems(); boolean enabled = false; for (int i = 0; i < items.length;
     * i++) { IContributionItem item = items[i]; enabled = item.isEnabled();
     * if(enabled) break; } // Workaround for 1GDDCN2: SWT:Linux -
    ...
    // // Workaround for 1GDDCN2: SWT:Linux - MenuItem.setEnabled() always
    // causes a redraw
