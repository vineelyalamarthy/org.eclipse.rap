 ...
 * items in tool bars. The controls for a command are built by some container,
 * which furnished the context where these controls appear and configures them
 * with data from properties declared by the action. When the end user triggers
 * the command via its control, the action's <code>run</code> method is
 * invoked to do the real work.
 * <p>
 ...
 * </p>
 * <p>
 * Clients should subclass the abstract base class <code>Action</code> to
 * define concrete actions rather than implementing <code>IAction</code> from
 * scratch.
 * </p>
 * <p>
 * This interface exists only to define the API for actions. It is not intended
...
public interface IAction {
  ...
  /**
   * Action style constant (value <code>0</code>) indicating action style is
   * not specified yet. By default, the action will assume a push button style.
   * If <code>setChecked</code> is called, then the style will change to a
   * check box, or if <code>setMenuCreator</code> is called, then the style
   * will change to a drop down menu.
   * 
   * @since 2.1
   */
  public static int AS_UNSPECIFIED = 0x00;
  /**
   * Action style constant (value <code>1</code>) indicating action is a
   * simple push button.
   */
  public static int AS_PUSH_BUTTON = 0x01;
  /**
   * Action style constant (value <code>2</code>) indicating action is a
   * check box (or a toggle button).
   */
  public static int AS_CHECK_BOX = 0x02;
  /**
   * Action style constant (value <code>4</code>) indicating action is a drop
   * down menu.
   */
  public static int AS_DROP_DOWN_MENU = 0x04;
  /**
   * Action style constant (value <code>8</code>) indicating action is a
   * radio button.
   * 
   * @since 2.1
   */
  ...
  /**
   * Property name of an action's enabled state (value <code>"enabled"</code>).
   */
   ...
   * Typically the description is shown as a (longer) help text in the status
   * line.
   */
  public static final String DESCRIPTION = "description"; //$NON-NLS-1$
  /**
   * Property name of an action's checked status (value <code>"checked"</code>).
   * Applicable when the style is <code>AS_CHECK_BOX</code> or
   * <code>AS_RADIO_BUTTON</code>.
   */
  public static final String CHECKED = "checked"; //$NON-NLS-1$
  /**
   * Property name of an action's success/fail result (value
   * <code>"result"</code>). The values are <code>Boolean.TRUE</code> if
   * running the action succeeded and <code>Boolean.FALSE</code> if running
   * the action failed or did not complete.
   * <p>
   * Not all actions report whether they succeed or fail. This property is
   * provided for use by actions that may be invoked by clients that can take
   * advantage of this information when present (for example, actions used in
   * cheat sheets). Clients should always assume that running the action
   * succeeded in the absence of notification to the contrary.
   * </p>
   * 
   * @since 3.0
   */
  ...
  /**
   * Property name of an action's handler. Some actions delegate some or all of
   * their behaviour or state to another object. In this case, if the object to
   * which behaviour has been delegated changes, then a property change event
   * should be sent with this name. This is used to support backward
   * compatibility of actions within the commands framework.
   * 
   * @since 3.1
   */
  ...
  /**
   * Adds a property change listener to this action. Has no effect if an
   ...
   * <code>MenuItem.getAccelerator</code>.
   * 
   ...
   * 
   * @return the action definition id of this action, or <code>null</code> if
   *         none
   * @since 2.0
   */
   ...
   * <p>
   * This method is associated with the <code>IMAGE</code> property; property
   * change events are reported when its value changes.
   * </p>
   * 
   ...
   * <p>
   * Hover images will be used on platforms that support changing the image when
   * the user hovers over the item. This method is associated with the
   * <code>IMAGE</code> property; property change events are reported when its
   * value changes.
   * </p>
   * 
   * @return the image, or <code>null</code> if this action has no image
   ...
   * <p>
   * This method is associated with the <code>TEXT</code> property; property
   * change events are reported when its value changes.
   * </p>
   * 
   ...
   * behaviour to some other object, then this method should answer whether such
   * an object is currently available.
   * 
   * @return <code>true</code> if all of the action's behaviour is available;
  ...
  /**
   * Removes the given listener from this action. Has no effect if an identical
  ...
  /**
   * Runs this action, passing the triggering SWT event. As of 2.0,
   * <code>ActionContributionItem</code> calls this method instead of
   * <code>run()</code>. The default implementation of this method in
   * <code>Action</code> simply calls <code>run()</code> for backwards
   * compatibility.
   * 
   ...
   * <code>AS_CHECK_BOX</code> or <code>AS_RADIO_BUTTON</code>.
   * <p>
   * Fires a property change event for the <code>CHECKED</code> property if
   * the checked status actually changes as a consequence.
   * </p>
   * 
   ...
   * </p>
   * 
   * @param text the description, or <code>null</code> to clear the
   *          description
   * @see #DESCRIPTION
   */
   ...
   * when the item is disabled.Fires a property change event for the
   * <code>IMAGE</code> property if the image actually changes as a
   * consequence.
   * </p>
   * 
   * @param newImage the image, or <code>null</code> if this action should not
   *          have an image
   * @see #IMAGE
   */
   ...
   * <p>
   * When an action is in the enabled state, the control associated with it is
   * active; triggering it will end up inkoking this action's <code>run</code>
   * method.
   * </p>
   * <p>
   * Fires a property change event for the <code>ENABLED</code> property if
   * the enabled state actually changes as a consequence.
   * </p>
   * 
   ...
   * <p>
   * Hover images will be used on platforms that support changing the image when
   * the user hovers over the item.Fires a property change event for the
   * <code>IMAGE</code> property if the image actually changes as a
   * consequence.
   * </p>
   * 
  ...
  /**
   * Sets the unique identifier for this action. This is used to identify
   * actions when added to a contribution manager. It should be set when the
   * action is created. It should not be modified once the action is part of an
   * action contribution item.
   * 
   ...
   * <p>
   * An accelerator specification may follow the actual text, separated from it
   * by an '@' or a '\t' character. An accelerator specification consists of
   * zero or more modifier tokens followed by a key code token. The tokens are
   ...
   * required for key bindings. If you set an accelerator using this method,
   * then it will not work in the workbench if it conflicts any existing key
   * binding, or if there is a different key binding defined for this action's
   * definition id. The definition id should be used instead -- referring to the
   * command in the workbench from which the key binding should be retrieved.
   * </p>
   * 
   * @param keycode the keycode to be accepted.
   */
