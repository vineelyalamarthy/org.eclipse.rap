...
public final class LegacyActionTools {
   ...
   * @return the RWT key code, or 0 if there is no accelerator
   */
  ...
  /**
   * Parses the given accelerator text, and converts it to an accelerator key
   * code. Support for localized modifiers is for backwards compatibility with
   * 1.0. Use setAccelerator(int) to set accelerators programatically or the
   ...
   * 
   * @param text The text from which the mnemonic should be extracted; may be
   *          <code>null</code>
  ...
  /**
   * Maps a standard keyboard key name to an RWT key code. Key names are
   ...
   * @return the RWT key code, <code>-1</code> if no match was found
   * @see RWT
   */
  ...
  /**
   * Maps an RWT key code to a standard keyboard key name. The key code is
   * stripped of modifiers (RWT.CTRL, RWT.ALT, RWT.SHIFT, and RWT.COMMAND). If
   * the key code is not an RWT code (for example if it a key code for the key
  ...
  public static final String findKeyString( final int keyCode ) {
    ...
    int i = keyCode & ~( RWT.CTRL | RWT.ALT | RWT.SHIFT | RWT.COMMAND );
   ...
   * to the current locale. Support for localized modifiers is for backwards
   * compatibility with 1.0. Use setAccelerator(int) to set accelerators
   * programatically or the <code>accelerator</code> tag in action definitions
   * in plugin.xml.
   * 
   ...
   * #findModifier. Support for localized modifiers is for backwards
   * compatibility with 1.0. Use setAccelerator(int) to set accelerators
   * programatically or the <code>accelerator</code> tag in action definitions
   * in plugin.xml.
   * 
  ...
  /**
   * Maps standard keyboard modifier key names to the corresponding RWT modifier
   * bit. The following modifier key names are recognized (case is ignored):
   * <code>"CTRL"</code>, <code>"SHIFT"</code>, <code>"ALT"</code>, and
   * <code>"COMMAND"</code>. The given modifier key name is converted to
   * upper case before comparison.
   * 
   * @param token the modifier key name
   * @return the RWT modifier bit, or <code>0</code> if no match was found
   * @see RWT
   */
  ...
  /**
   * Returns a string representation of an RWT modifier bit (RWT.CTRL, RWT.ALT,
   * RWT.SHIFT, and RWT.COMMAND). Returns <code>null</code> if the key code is
   * not an RWT modifier bit.
   * 
   * @param keyCode the RWT modifier bit to be translated
   * @return the string representation of the RWT modifier bit, or
   *         <code>null</code> if the key code was not an RWT modifier bit
   * @see RWT
   */
...
//    }
  ...
  private static String getModifierString( int keyCode ) {
    ...
    if( ( keyCode & RWT.CTRL ) != 0 ) {
      modString = findModifierString( keyCode & RWT.CTRL );
    }
    if( ( keyCode & RWT.ALT ) != 0 ) {
      if( modString.equals( "" ) ) { //$NON-NLS-1$
        modString = findModifierString( keyCode & RWT.ALT );
      } else {
        modString = modString + "+" + findModifierString( keyCode & RWT.ALT ); //$NON-NLS-1$
      }
    }
    if( ( keyCode & RWT.SHIFT ) != 0 ) {
      if( modString.equals( "" ) ) { //$NON-NLS-1$
        modString = findModifierString( keyCode & RWT.SHIFT );
      } else {
        modString = modString + "+" + findModifierString( keyCode & RWT.SHIFT ); //$NON-NLS-1$
      }
    }
    if( ( keyCode & RWT.COMMAND ) != 0 ) {
      if( modString.equals( "" ) ) { //$NON-NLS-1$
        modString = findModifierString( keyCode & RWT.COMMAND );
      } else {
        modString = modString
                    + "+" + findModifierString( keyCode & RWT.COMMAND ); //$NON-NLS-1$
      }
    }
    return modString;
  }
...
//    keyCodes.put( "ARROW_UP", new Integer( RWT.ARROW_UP ) ); //$NON-NLS-1$
//    keyCodes.put( "ARROW_DOWN", new Integer( RWT.ARROW_DOWN ) ); //$NON-NLS-1$
//    keyCodes.put( "ARROW_LEFT", new Integer( RWT.ARROW_LEFT ) ); //$NON-NLS-1$
//    keyCodes.put( "ARROW_RIGHT", new Integer( RWT.ARROW_RIGHT ) ); //$NON-NLS-1$
//    keyCodes.put( "PAGE_UP", new Integer( RWT.PAGE_UP ) ); //$NON-NLS-1$
//    keyCodes.put( "PAGE_DOWN", new Integer( RWT.PAGE_DOWN ) ); //$NON-NLS-1$
//    keyCodes.put( "HOME", new Integer( RWT.HOME ) ); //$NON-NLS-1$
//    keyCodes.put( "END", new Integer( RWT.END ) ); //$NON-NLS-1$
//    keyCodes.put( "INSERT", new Integer( RWT.INSERT ) ); //$NON-NLS-1$
//    keyCodes.put( "F1", new Integer( RWT.F1 ) ); //$NON-NLS-1$
//    keyCodes.put( "F2", new Integer( RWT.F2 ) ); //$NON-NLS-1$
//    keyCodes.put( "F3", new Integer( RWT.F3 ) ); //$NON-NLS-1$
//    keyCodes.put( "F4", new Integer( RWT.F4 ) ); //$NON-NLS-1$
//    keyCodes.put( "F5", new Integer( RWT.F5 ) ); //$NON-NLS-1$
//    keyCodes.put( "F6", new Integer( RWT.F6 ) ); //$NON-NLS-1$
//    keyCodes.put( "F7", new Integer( RWT.F7 ) ); //$NON-NLS-1$
//    keyCodes.put( "F8", new Integer( RWT.F8 ) ); //$NON-NLS-1$
//    keyCodes.put( "F9", new Integer( RWT.F9 ) ); //$NON-NLS-1$
//    keyCodes.put( "F10", new Integer( RWT.F10 ) ); //$NON-NLS-1$
//    keyCodes.put( "F11", new Integer( RWT.F11 ) ); //$NON-NLS-1$
//    keyCodes.put( "F12", new Integer( RWT.F12 ) ); //$NON-NLS-1$
  }
...
//    keyStrings.put( new Integer( SWT.ARROW_UP ),
//                    JFaceResources.getString( "Arrow_Up" ) ); //$NON-NLS-1$
//    keyStrings.put( new Integer( SWT.ARROW_DOWN ),
//                    JFaceResources.getString( "Arrow_Down" ) ); //$NON-NLS-1$
//    keyStrings.put( new Integer( SWT.ARROW_LEFT ),
//                    JFaceResources.getString( "Arrow_Left" ) ); //$NON-NLS-1$
//    keyStrings.put( new Integer( SWT.ARROW_RIGHT ),
//                    JFaceResources.getString( "Arrow_Right" ) ); //$NON-NLS-1$
//    keyStrings.put( new Integer( SWT.PAGE_UP ),
//                    JFaceResources.getString( "Page_Up" ) ); //$NON-NLS-1$
//    keyStrings.put( new Integer( SWT.PAGE_DOWN ),
//                    JFaceResources.getString( "Page_Down" ) ); //$NON-NLS-1$
...
//    keyStrings.put( new Integer( SWT.INSERT ),
//                    JFaceResources.getString( "Insert" ) ); //$NON-NLS-1$
