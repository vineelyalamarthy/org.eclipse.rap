...
public class ActionContributionItem extends ContributionItem {
   ...
   * mode bit is not set, text is only shown on tool items if there is no image
   * present.
   * 
   * @since 3.0
   */
...
//  private Listener toolItemListener;
  ...
  // TODO: [fappel] this is only a first approach, since RWT doesn't support
  //                untyped listeners. I think it should be possible to use
  //                only a selection and a dispose listener
  private class ButtonListener implements SelectionListener, DisposeListener {
  ...
  private class MenuItemListener implements SelectionListener, DisposeListener {
...
//      b.addListener( SWT.Dispose, getButtonListener() );
      ...
      b.addSelectionListener( getButtonListener() );
...
//      mi.addListener( SWT.Dispose, getMenuItemListener() );
      mi.addSelectionListener( getMenuItemListener() );
...
//      ti.addListener( SWT.Selection, getToolItemListener() );
      ti.addSelectionListener( getToolItemListener() );
  ...
  private ButtonListener getButtonListener() {
    if( buttonListener == null ) {
      buttonListener = new ButtonListener() {
        public void widgetSelected( final SelectionEvent event ) {
          Widget ew = ( Widget )event.getSource();
          if( ew != null ) {
            handleWidgetSelection( event, ( ( Button )ew ).getSelection() );
          }
        }
        // TODO: [fappel] override widget disposal, think about refacting with
        //                only two listeners for selection and disposal
      };
    }
...
//  }
   ...
   */
  private MenuItemListener getMenuItemListener() {
   if( menuItemListener == null ) {
     menuItemListener = new MenuItemListener() {
       public void widgetSelected( final SelectionEvent event ) {
         Widget ew = ( Widget )event.getSource();
         if( ew != null ) {
           handleWidgetSelection( event, false /*( ( MenuItem )ew ).getSelection()*/ );
         }
       }
       // TODO: [fappel] override widget disposal, think about refacting with
       //                only two listeners for selection and disposal
...
//  }
   ...
   */
  private ToolItemListener getToolItemListener() {
    if( toolItemListener == null ) {
      toolItemListener = new ToolItemListener() {
        public void widgetSelected( final SelectionEvent event ) {
          Widget ew = ( Widget )event.getSource();
          if( ew != null ) {
...
//  }
   ...
   */
  private void handleWidgetSelection( SelectionEvent e, boolean selection ) {
    Widget item = ( Widget )e.getSource();
    if( item != null ) {
      int style = item.getStyle();
      if( ( style & ( RWT.TOGGLE | RWT.CHECK ) ) != 0 ) {
        if( action.getStyle() == IAction.AS_CHECK_BOX ) {
...
//        action.runWithEvent( e );
        // TODO: [fappel] support runWithEvent
        action.run();
//        if( trace ) {
...
//  }
  ...
  public boolean isDynamic() {
    if( widget instanceof MenuItem ) {
      // Optimization. Only recreate the item is the check or radio style has
      // changed.
      boolean itemIsCheck = ( widget.getStyle() & RWT.CHECK ) != 0;
      boolean actionIsCheck = getAction() != null
                              && getAction().getStyle() == IAction.AS_CHECK_BOX;
      boolean itemIsRadio = ( widget.getStyle() & RWT.RADIO ) != 0;
...
//    return super.isVisible() && isCommandActive();
   ...
   * 
   * @param propertyName the name of the property, or <code>null</code>
  ...
  public void update( String propertyName ) {
    if( widget != null ) {
      ...
      boolean checkChanged = ( action.getStyle() == IAction.AS_CHECK_BOX || action.getStyle() == IAction.AS_RADIO_BUTTON )
      ...
      if( widget instanceof ToolItem ) {
        ...
        if( textChanged ) {
          String textToSet = showText
                                     ? text
                                     : ""; //$NON-NLS-1$
          boolean rightStyle = ( ti.getParent().getStyle() & RWT.RIGHT ) != 0;
...
//          }
   ...
   *          and <code>false</code> if it is acceptable for this item to have
   *          no image
...
//        ( ( ToolItem )widget ).setImage( image == null
//                                                      ? null
...
//        image = ImageDescriptor.createWithFlags( action.getHoverImageDescriptor(),
//                                                 SWT.IMAGE_GRAY );
//      } else {
...
//        image = ImageDescriptor.getMissingImageDescriptor();
//      }
//      // Create a local resource manager to remember the images we've allocated
//      // for this tool item
//      LocalResourceManager localManager = new LocalResourceManager( parentResourceManager );
...
//                                                                    ? null
//                                                                    : localManager.createImageWithDefault( disabledImage ) );
//      ( ( ToolItem )widget ).setHotImage( hoverImage == null
//                                                            ? null
...
//        ( ( Item )widget ).setImage( image == null
//                                                  ? null
//                                                  : localManager.createImageWithDefault( image ) );
//      } else if( widget instanceof Button ) {
//        ( ( Button )widget ).setImage( image == null
//                                                    ? null
//                                                    : localManager.createImageWithDefault( image ) );
//      }
//      // Now that we're no longer referencing the old images, clear them out.
