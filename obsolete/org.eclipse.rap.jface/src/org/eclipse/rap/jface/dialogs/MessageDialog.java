...
public class MessageDialog extends IconAndMessageDialog {
   ...
   * constructor as an array. The <code>open</code> method will return the
   * index of the label in this array corresponding to the button that was
   * pressed to close the dialog. If the dialog was dismissed without pressing a
   * button (ESC, etc.) then -1 is returned.
   * </p>
   * 
   * <p>
   * <strong>Important:</strong> Note that, in contrast to SWT, the
   * <code>open</code> method does not block. To be able to react on the
   * closing of the dialog window and to get the return value, an
   * {@link IWindowCallback} object must be passed to <code>open</code>.
   * </p>
   * 
  ...
  protected Control createDialogArea( Composite parent ) {
    ...
    if( customArea == null ) {
      customArea = new Label( composite, RWT.NULL );
    }
    return composite;
  }
   ...
   * 
   * <p>
   * <strong>Important:</strong> In contrast to SWT, this method does not block.
   * To get the return value, a {@link IWindowCallback} must be passed.
   * Possible return values passed to the callback: <code>OK</code>,
   * <code>Cancel</code>.
   * </p>
   * 
   ...
   * 
   * @param callback The callback to be called when the shell is closed.
   */
  ...
  {
    Image dialogImage = null; // accept the default window icon
    String[] dialogButtons = new String[]{
      IDialogConstants.OK_LABEL, IDialogConstants.CANCEL_LABEL
    };
    int defaultButton = 0; // ok is the default
                                   ...
                                   IWindowCallback callback )
  {
    ...
    int defaultButton = 0; // yes is the default
    MessageDialog dialog = new MessageDialog( parent,
                                              title,
                                              dialogImage,
                                              message,
  ...
  protected boolean customShouldTakeFocus() {
    ...
    //        TODO [rst] uncomment when CLabel is implemented
    //        if (customArea instanceof CLabel) {
    //            return (customArea.getStyle() & RWT.NO_FOCUS) > 0;
    //        }
    return true;
  }
