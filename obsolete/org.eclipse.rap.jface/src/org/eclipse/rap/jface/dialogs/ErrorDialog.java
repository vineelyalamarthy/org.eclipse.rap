...
//  private Clipboard clipboard;
   ...
   */
  public ErrorDialog( final Shell parentShell,
                      ...
                      final int displayMask )
  {
    ...
    setShellStyle( getShellStyle() | RWT.RESIZE );
  }

  /*
   * (non-Javadoc) Method declared on Dialog. Handles the pressing of the Ok or
   * Details button in this dialog. If the Ok button was pressed then close this
   * dialog. If the Details button was pressed then toggle the displaying of the
   * error details area. Note that the Details button will only be visible if
   * the error being displayed specifies child details.
   */
  ...
  protected void createDialogAndButtonArea( final Composite parent ) {
    ...
    if( this.dialogArea instanceof Composite ) {
      ...
      if( dialogComposite.getChildren().length == 0 ) {
        new Label( dialogComposite, RWT.NULL );
      }
    }
  }
...
//    TODO [rst] uncomment when getItemheight exists
//    data.heightHint = list.getItemHeight() * LIST_ITEM_COUNT;
    data.heightHint = 12 * LIST_ITEM_COUNT;
    data.horizontalSpan = 2;
    list.setLayoutData( data );
    list.setFont( parent.getFont() );
//    Menu copyMenu = new Menu( list );
//    MenuItem copyItem = new MenuItem( copyMenu, RWT.NONE );
...
//    list.setMenu( copyMenu );
   ...
   * error. If you specified a mask to filter the displaying of these children,
   * the error dialog will only be displayed if there is at least one child
   ...
   *          none
   * @param dialogTitle the title to use for this dialog, or <code>null</code>
   *          to indicate that the default title should be used
   * @param message the message to show in this dialog, or <code>null</code>
   *          to indicate that the error's message should be shown as the
   *          primary message
   * @param status the error to show to the user
   */
  public static void openError( final Shell parent,
                                final String dialogTitle,
                                final String message,
                                final IStatus status,
                                final IWindowCallback callback )
  {
    ...
    openError( parent, dialogTitle, message, status, mask, callback );
  }
   ...
   * 
   * @param parentShell the parent shell of the dialog, or <code>null</code>
   *          if none
   ...
   * @see org.eclipse.core.runtime.IStatus#matches(int)
   */
  public static void openError( final Shell parentShell,
                                final String title,
                                final String message,
                                final IStatus status,
                                final int displayMask,
                                final IWindowCallback callback )
  {
    ErrorDialog dialog = new ErrorDialog( parentShell,
                                          title,
                                          message,
                                          status,
                                          displayMask );
   ...
   * child static of the status object and displays them in a list. The format
   ...
   * @param currNesting the nesting level (increases one level for each level of
   *          children)
   * @param includeStatus whether to include the buildingStatus in the display
   *          or just its children
   */
  private void populateList( final List listToPopulate,
                             final IStatus buildingStatus,
                             final int nesting,
                             final boolean includeStatus )
  {
    if( buildingStatus.matches( displayMask ) ) {
      ...
      int currNesting = nesting;
      if( includeStatus ) {
        StringBuffer sb = new StringBuffer();
        for( int i = 0; i < currNesting; i++ ) {
      ...
      if( isCoreException ) {
        ...
        // Only print the exception message if it is not contained in the parent
        // message
        if( message == null || message.indexOf( eStatus.getMessage() ) == -1 ) {
          populateList( listToPopulate, eStatus, currNesting, true );
        }
      }
      // Look for child status
      IStatus[] children = buildingStatus.getChildren();
      for( int i = 0; i < children.length; i++ ) {
        populateList( listToPopulate, children[ i ], currNesting, true );
      }
    }
  }
  ...
  private void toggleDetailsArea() {
    ...
    Point oldSize = getShell().computeSize( RWT.DEFAULT, RWT.DEFAULT );
    if( listCreated ) {
      list.dispose();
      listCreated = false;
    ...
    Point newSize = getShell().computeSize( RWT.DEFAULT, RWT.DEFAULT );
    getShell().setSize( new Point( windowSize.x, windowSize.y
                                                 + ( newSize.y - oldSize.y ) ) );
  }
...
//  }
   ...
   * of the dialog. Invoking the method before the content area has been set or
   * after the dialog has been disposed will have no effect.
   */
