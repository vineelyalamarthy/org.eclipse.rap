  ...
  implements ICoolBarManager
{
  ...
  protected boolean allowItem( IContributionItem itemToAdd ) {
    /*
     * We will allow as many null entries as they like, though there should be
     * none.
     */
    if( itemToAdd == null ) {
      return true;
...
//        }
  ...
  private CoolItem findCoolItem( IContributionItem item ) {
    CoolItem[] coolItems = ( coolBar == null )
                                              ? null
                                              : coolBar.getItems();
  ...
  /**
   * Returns the control of the Menu Manager. If the menu manager does not have
   * a control then one is created.
   * 
   ...
   * next on the returned iterator, it will return the first element of the next
   * row.
   * 
...
//    }
    ...
    if( contributionList.size() != 0 ) {
      ...
      array = (org.eclipse.jface.action.IContributionItem[] )contributionList.toArray( array );
   ...
   * @param itemLocation
   */
  private void relocate( IContributionItem cbItem,
                         int index,
                         ArrayList contributionList,
...
//      coolBar.setRedraw( false );
       ...
       * Separators should stay because they mark line breaks in a cool bar.
       */
      ...
      /*
       * Make a list of CoolItem widgets in the cool bar for which there is no
       * current visible contribution item. These are the widgets to be
       * disposed. Dynamic items are also removed.
       */
      CoolItem[] coolItems = coolBar.getItems();
  ...
  /* package */void updateTabOrder() {
    if( coolBar != null ) {
      ...
      if( items != null ) {
        ...
        childrenArray = (org.eclipse.swt.widgets.Control[] )children.toArray( childrenArray );
