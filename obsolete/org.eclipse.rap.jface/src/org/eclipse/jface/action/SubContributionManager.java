...
/**
 * A <code>SubContributionManager</code> is used to define a set of
 * contribution items within a parent manager. Once defined, the visibility of
 * the entire set can be changed as a unit.
 */
public abstract class SubContributionManager implements IContributionManager {
  ...
  /**
   * Maps each item in the manager to a wrapper. The wrapper is used to control
   * the visibility of each item.
   */
   ...
   *          <code>SubContributionManager</code> are forwarded and appear in
   *          the parent manager.
   */
  ...
  /**
   * Disposes this sub contribution manager, removing all its items and cleaning
   * up any other resources allocated by it. This must leave no trace of this
   * sub contribution manager in the parent manager. Subclasses may extend.
   * 
   * @since 3.0
   */
  ...
  /*
   * (non-Javadoc) Method declared on IContributionManager. Returns the item
   * passed to us, not the wrapper.
   */
  ...
  /*
   * (non-Javadoc) Method declared on IContributionManager. Returns the items
   * passed to us, not the wrappers.
   */
   ...
   * @param wrap the item contributed to the parent manager as a proxy for the
   *          item contributed by the client
   */
  ...
  /**
   * Wraps a contribution item in a sub contribution item, and returns the new
   * wrapper.
   */
  ...
  /**
   * Unwraps a nested contribution item. If the contribution item is an instance
   * of <code>SubContributionItem</code>, then its inner item is returned.
   * Otherwise, the item itself is returned.
   * 
   * @param item The item to unwrap; may be <code>null</code>.
   * @return The inner item of <code>item</code>, if <code>item</code> is a
   *         <code>SubContributionItem</code>;<code>item</code> otherwise.
   */
