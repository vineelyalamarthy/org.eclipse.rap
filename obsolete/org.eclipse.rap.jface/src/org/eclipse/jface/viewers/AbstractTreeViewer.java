...
public abstract class AbstractTreeViewer extends StructuredViewer {
   ...
   * viewers to support multiple equal elements. See bug 76482 for more details.
   * This support will likely be removed in Eclipse 3.2 in favour of proper
   * support for multiple equal elements.
   * </p>
   * 
   * @param widget the widget for the parent element
   * @param parentElementOrTreePath the parent element
   * @param childElements the child elements to add
   * @since 3.1
   */
  ...
  private void createAddedElements( Widget widget, Object[] elements ) {
    ...
    for( int i = 0; i < elements.length; i++ ) {
      ...
      if( comparator == null ) {
        ...
        lastInsertion = insertionPosition( items,
                                           comparator,
                                           lastInsertion,
                                           element,
                                           parentPath );
        // As we are only searching the original array we keep track of those
        // positions only
        if( lastInsertion == items.length ) {
          index = -1;
        } else {// See if we should just refresh
          while( lastInsertion < items.length
                 && internalCompare( comparator,
                                     parentPath,
                                     element,
                                     items[ lastInsertion ].getData() ) == 0 )
          {
            // As we cannot assume the sorter is consistent with equals() -
            // therefore we can
   ...
   *          allows optimising search for multiple elements that are sorted
   *          themself.
   * @param element element to find position for.
   * @param parentPath the tree path for the element's parent or
   *          <code>null</code> if the element is a root element or the sorter
   *          is not a {@link TreePathViewerSorter}
   * @return the index to use when inserting the element.
   */
  private int insertionPosition( Item[] items,
   ...
   * @param sorter The sorter to use.
   * @param startIndex the start index to start search for position from this
   *          allows optimising search for multiple elements that are sorted
   *          themself.
   * @param element element to find position for.
   ...
   * {@link TreePathViewerSorter}, the provided tree path will be used. If the
   * tree path is null and the sorter is a tree path sorter, then the elements
   * are root elements
   * 
   * @param sorter the sorter
   ...
   * accurately reflect the model. This method only affects the viewer, not the
   * model. Note that there is another method for efficiently processing the
   * simultaneous addition of multiple elements.
   * </p>
   * 
   * @param parentElementOrTreePath the parent element or path
  ...
  public void add( Object parentElementOrTreePath, Object childElement ) {
    add( parentElementOrTreePath, new Object[]{
      childElement
    } );
  }
...
//  }
  ...
  /**
   * Get the child for the widget at index. Note that the default implementation
   * is not very effecient and should be overridden if this class is
   * implemented.
   * 
  ...
  /**
   * Returns a list of elements corresponding to expanded nodes in this viewer's
   * tree, including currently hidden ones that are marked as expanded but are
  ...
  /**
   * Returns whether the node corresponding to the given element or tree path is
   ...
   * 
   * @param item the item
   * @param includeChildren <code>true</code> if the children are considered
   *          in determining which item is next, and <code>false</code> if
...
//    }
  ...
  /*
   * (non-Javadoc) Method declared on StructuredViewer. Builds the initial tree
   * and handles the automatic expand feature.
   */
  ...
  protected void internalCollapseToLevel( Widget widget, int level ) {
    if( level == ALL_LEVELS || level > 0 ) {
      ...
      if( level == ALL_LEVELS || level > 1 ) {
        ...
        if( children != null ) {
          int nextLevel = ( level == ALL_LEVELS
                                               ? ALL_LEVELS
                                               : level - 1 );
          for( int i = 0; i < children.length; i++ ) {
   ...
   * 
   * @param result a list (element type: <code>Item</code>) into which to
   ...
   * 
   * @param elementOrPath the element
   * @param expand <code>true</code> if all nodes on the path should be
  ...
  /**
   * If the argument is a tree path, returns its last segment, otherwise return
   * the argument
   * 
   ...
   * @param widget the widget
   * @param element the element
   * @param doStruct <code>true</code> if structural changes are to be picked
   *          up, and <code>false</code> if only label provider changes are of
   *          interest
   * @param updateLabels <code>true</code> to update labels for existing
   *          elements, <code>false</code> to only update labels as needed,
   *          assuming that labels for existing elements are unchanged.
   * @since 3.1
   */
  protected void internalRefresh( Widget widget,
                                  Object element,
                                  boolean doStruct,
   ...
   * @param updateLabels
   */
  /* package */void internalRefreshStruct( Widget widget,
                                            Object element,
                                            boolean updateLabels )
  {
   ...
   * </p>
   * 
   * @param elementsOrPaths the elements or element paths to remove
   * @since 3.1
   */
  ...
  /**
   * Removes the given elements from this viewer, whenever those elements appear
   ...
   * expanded elements.
   * 
   * @param expandedElements the set (element type: <code>Object</code>) of
   *          elements which are expanded
   * @param widget the widget
   */
   ...
   * expanded tree paths.
   * 
   * @param expandedTreePaths the set (element type: <code>TreePath</code>)
   *          of elements which are expanded
   * @param widget the widget
   */
  private void internalSetExpandedTreePaths( CustomHashtable expandedTreePaths,
                                             Widget widget,
                                             TreePath currentPath )
  {
    ...
    for( int i = 0; i < items.length; i++ ) {
      Item item = items[ i ];
      Object data = item.getData();
      TreePath childPath = data == null
                                       ? null
                                       : currentPath.createChildPath( data );
   ...
   * expanded. Clients should query expandablity by path if the viewer's content
   * provider is an {@link ITreePathContentProvider}.
   * <p>
   ...
   * </p>
   * 
   * @param elementOrTreePath the element or path
   * @return <code>true</code> if the tree node representing the given element
   *         can be expanded, or <code>false</code> if not
   */
   ...
   * @param parentPath the parent path if it is knwon or <code>null</code> if
   *          it needs to be determines
   * @param element the element
   * @return whether the given element is expandable
   */
   ...
   * children of the given parent element. If the given elements also appear as
   * children of some other parent, the other parent will remain unchanged. The
   * selection is updated if required.
   * <p>
   * This method should be called (by the content provider) when elements have
   * been removed from the model, in order to cause the viewer to accurately
   * reflect the model. This method only affects the viewer, not the model.
   ...
   * accurately reflect the model. This method only affects the viewer, not the
   * model. Note that there is another method for efficiently processing the
   * simultaneous removal of multiple elements.
   * </p>
   * 
   * @param elementsOrTreePaths the element
   */
  ...
  /**
   * Sets the auto-expand level. The value 0 means that there is no auto-expand;
   * 1 means that top-level elements are expanded, but not their children; 2
   * means that top-level elements are expanded, and their children, but not
   ...
   * to ensure that the content provider is an <code>ITreeContentProvider</code>.
   */
  ...
  /**
   * Sets which nodes are expanded in this viewer's tree. The given list
   * contains the tree paths that are to be expanded; all other nodes are to be
   * collapsed.
   * <p>
   * This method is typically used when restoring the interesting state of a
   ...
   * </p>
   * 
   * @param treePaths the array of expanded tree paths
  ...
  public void setExpandedTreePaths( TreePath[] treePaths ) {
    ...
    CustomHashtable expandedTreePaths = new CustomHashtable( treePaths.length * 2 + 1,
                                                             treePathComparer );
    ...
    // equal elements, and those are in the set of elements to be expanded,
    // only the first item found for each element will be expanded.
    internalSetExpandedTreePaths( expandedTreePaths,
                                  getControl(),
   ...
   * 
   * @param items list of items (element type:
   *          <code>org.eclipse.swt.widgets.Item</code>)
   */
  protected abstract void setSelection( List items );

  /**
   * This implementation of setSelectionToWidget accepts a list of elements or a
   * list of tree paths.
   */
  ...
  /**
   * Updates the "+"/"-" icon of the tree node from the given element. It calls
   * <code>isExpandable</code> to determine whether an element is expandable.
   * 
   * @param item the item
   * @param element the element
   */
  ...
  public ISelection getSelection() {
    ...
    return new TreeSelection( (org.eclipse.jface.viewers.TreePath[] )list.toArray( new TreePath[ list.size() ] ),
                              getComparer() );
  }
   ...
   * viewer's tree, including currently hidden ones that are marked as expanded
   * but are under a collapsed ancestor.
   * <p>
   * This method is typically used when preserving the interesting state of a
   * viewer; <code>setExpandedElements</code> is used during the restore.
   * </p>
  ...
  public TreePath[] getExpandedTreePaths() {
    ...
    return (org.eclipse.jface.viewers.TreePath[] )result.toArray( new TreePath[ items.size() ] );
  }
   ...
   * element at the given position. If this viewer has a sorter, the position is
   * ignored and the element is inserted at the correct position in the sort
   * order.
   * <p>
   * This method should be called (by the content provider) when elements have
   * been added to the model, in order to cause the viewer to accurately reflect
   ...
   * </p>
   * 
   * @param parentElementOrTreePath the parent element, or the tree path to the
   *          parent
   * @param element the element
   ...
   * @since 3.2
   */
  public void insert( Object parentElementOrTreePath,
                      Object element,
                      int position )
  {
    Assert.isNotNull( parentElementOrTreePath );
