 ...
 * @since 3.2
 */
public class TreeSelection extends StructuredSelection
  implements ITreeSelection
{

  /*
   * Implementation note. This class extends StructuredSelection because many
   * pre-existing JFace viewer clients assumed that the only implementation of
   * IStructuredSelection was StructuredSelection. By extending
   * StructuredSelection rather than implementing ITreeSelection directly, we
   * avoid this problem. For more details, see Bug 121939 [Viewers]
   * TreeSelection should subclass StructuredSelection.
   */
   ...
   * 
   * @param paths tree paths
   * @param comparer the comparer, or <code>null</code> if default equals is
   *          to be used
   */
   ...
   * path.
   * 
   * @param treePath tree path, or <code>null</code> for an empty selection
   */
  ...
  /**
   * Returns the element comparer passed in when the tree selection has been
   * created or <code>null</code> if no comparer has been provided.
   * 
  ...
  public TreePath[] getPaths() {
    return paths == null
                        ? EMPTY_TREE_PATHS
                        : (org.eclipse.jface.viewers.TreePath[] )paths.clone();
  }

  public TreePath[] getPathsFor( Object element ) {
    Object value = element2TreePaths == null
                                            ? null
                                            : element2TreePaths.get( element );
    if( value == null ) {
      ...
      List l = ( List )value;
      return (org.eclipse.jface.viewers.TreePath[] )l.toArray( new TreePath[ l.size() ] );
    } else {
