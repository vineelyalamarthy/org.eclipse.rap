  ...
  implements IPostSelectionProvider
{
   ...
   * <code>Widget[]</code>). <code>null</code> means that the element map
   * is disabled.
   */
   ...
   * IColorProvider, the IFontProvider and/or the IViewerLabelProvider for color
   * and font updates. Initialize it to have no color or font providing
   * initially.
   * 
   * @since 3.1
   */
  ...
  /**
   * The ColorAndFontCollector is a helper class for viewers that have color and
   * font support ad optionally decorators.
   * 
   * @see IColorDecorator
   * @see IFontDecorator
   * @see IColorProvider
    ...
    extends ColorAndFontCollector
  {
     ...
     * If it is an IColorProvider or IFontProvider set these values up.
     * 
  ...
  /**
   * The ColorAndFontManager collects fonts and colors without a a color or font
   * provider.
   */
  protected class ColorAndFontCollector {
    ...
    /**
     * Create a new instance of the receiver with no colour and font provider.
     */
  ...
  /**
   * Adds support for dragging items out of this viewer via a user drag-and-drop
   * operation.
   * 
   * @param operations a bitwise OR of the supported drag and drop operation
   *          types ( <code>DROP_COPY</code>,<code>DROP_LINK</code>, and
   *          <code>DROP_MOVE</code>)
   * @param transferTypes the transfer types that are supported by the drag
   *          operation
   * @param listener the callback that will be invoked to set the drag data and
   *          to cleanup after the drag and drop operation finishes
   ...
   *          types ( <code>DROP_COPY</code>,<code>DROP_LINK</code>, and
   *          <code>DROP_MOVE</code>)
   * @param transferTypes the transfer types that are supported by the drop
   *          operation
   * @param listener the callback that will be invoked after the drag and drop
   *          operation finishes
   * @see org.eclipse.swt.dnd.DND
   */
   ...
   * data to be the element, and maps the element to the item in the element map
   * (if enabled).
   * 
   ...
   * from element to widgets is removed and the new mapping is added. If fullmap
   ...
   * @param fullMap <code>true</code> if mappings are added and removed, and
  ...
  protected boolean equals( Object elementA, Object elementB ) {
    if( comparer == null ) {
      ...
      return elementA == null
                             ? elementB == null
                             : comparer.equals( elementA, elementB );
    }
  }
    ...
    if( filters != null ) {
      ...
      for( int i = 0; i < elements.length; i++ ) {
        ...
        for( int j = 0; j < filters.size(); j++ ) {
          add = ( ( ViewerFilter )filters.get( j ) ).select( this,
                                                             root,
                                                             elements[ i ] );
          if( !add ) {
            break;
   ...
   * for the given element assuming that it is the viewer's input; this is done
   * by calling <code>doFindInputItem</code>. If it is not found there, it is
   * looked up in the internal element map provided that this feature has been
   * enabled. If the element map is disabled, the widget is found via
   ...
   * must not be changed by clients; it might change upon calling other methods
   * on this viewer.
   * <p>
   ...
   * The default implementation of this method tries first to find the widget
   * for the given element assuming that it is the viewer's input; this is done
   * by calling <code>doFindInputItem</code>. If it is not found there, the
   * widgets are looked up in the internal element map provided that this
  ...
  protected final Widget[] findItems( Object element ) {
    ...
    if( usingElementMap() ) {
      ...
      if( widgetOrWidgets == null ) {
        ...
        return new Widget[]{
          ( Widget )widgetOrWidgets
        };
      } else {
        return (org.eclipse.swt.widgets.Widget[] )widgetOrWidgets;
      }
    }
  ...
  // }
  /**
   * Notifies any open event listeners that a open event has been received. Only
  ...
  // }
  /**
   * Notifies any post selection listeners that a post selection event has been
  ...
  /**
   * Returns the filtered array of children of the given element. The resulting
   * array must not be modified, as it may come directly from the model's
   * internal state.
   * 
   ...
   * either remove the element from the selection or to pick another element as
   * its new selection. The default implementation of this method calls
   ...
   * @param newSelection the selection after the update, or <code>null</code>
   *          if none
   */
   ...
   * event specifies that the label of a given element has changed, otherwise it
   * calls super. Subclasses may reimplement or extend.
   * </p>
   * 
...
//    } );
  ...
  /**
   * Refreshes this viewer starting at the given element. Labels are updated as
   * described in <code>refresh(boolean updateLabels)</code>.
   * <p>
  ...
  protected void mapElement( Object element, Widget item ) {
    if( elementMap != null ) {
      ...
      if( widgetOrWidgets == null ) {
        ...
        Widget[] widgets = (org.eclipse.swt.widgets.Widget[] )widgetOrWidgets;
        int indexOfItem = Arrays.asList( widgets ).indexOf( item );
        if( indexOfItem == -1 ) {
          int length = widgets.length;
          System.arraycopy( widgets,
                            0,
  ...
  /**
   * Returns a new hashtable using the given capacity and this viewer's element
   * comparer.
   * 
  ...
  /**
   * Refreshes this viewer with information freshly obtained from this viewer's
   * model. If <code>updateLabels</code> is <code>true</code> then labels
   * for otherwise unaffected elements are updated as well. Otherwise, it
   * assumes labels for existing elements are unchanged, and labels are only
   * obtained as needed (for example, for new elements).
   * <p>
   ...
   * @param reveal <code>true</code> to reveal the first element in the
  ...
  /**
   * Sets this viewer's comparator to be used for sorting elements, and triggers
   * refiltering and resorting of this viewer's element. <code>null</code>
   * turns sorting off. To get the viewer's comparator, call
   * <code>getComparator()</code>.
   * <p>
   * IMPORTANT: This method was introduced in 3.2. If a reference to this viewer
   * object is passed to clients who call
   * <code>getSorter()<code>, null may be returned from
   ...
   * 
   * @param enable <code>true</code> to enable hash lookup, and
   ...
   * use the default <code>equals</code> and <code>hashCode</code> methods
   ...
   * </p>
   * <p>
   * This should be called when an element has changed in the model, in order to
   * have the viewer accurately reflect the model. This method only affects the
   ...
   * properties, elements may appear or disappear if the change affects whether
   * or not they are filtered out.
   * </p>
   * 
   * @param elements the elements
   * @param properties the properties that have changed, or <code>null</code>
   *          to indicate unknown
   */
  ...
  /**
   * Updates the given element's presentation when one or more of its properties
   * changes. Only the given element is updated.
   * <p>
   * This does not handle structural changes (e.g. addition or removal of
   * elements), and does not update any other related elements (e.g. child
   * elements). To handle structural changes, use the <code>refresh</code>
   ...
   * This support will likely be removed in Eclipse 3.3 in favour of proper
   * support for multiple equal elements (which was implemented for
   * AbtractTreeViewer in 3.2).
   * </p>
   * 
   * @param widget the widget for the element
   * @param element the element
   * @param properties the properties that have changed, or <code>null</code>
   *          to indicate unknown
   */
   ...
   * listeners recorded in an internal state variable. Overriding this method is
  ...
  /**
   * Returns whether this structured viewer is configured to use an internal map
   * to speed up the mapping between elements and SWT items.
   * <p>
   ...
   */
  void buildLabel( ViewerLabel updateLabel,
                   Object element,
