 ...
 * Peter Shipton - original hashtable implementation Nick Edgar - added element
 * comparer support
 ******************************************************************************/
...
/* package */final class CustomHashtable {
  ...
  /**
   * Constructs a new hash table with the default capacity and the given element
   * comparer.
   * 
   * @param comparer the element comparer to use to compare keys and obtain hash
  ...
  public CustomHashtable( int capacity, IElementComparer comparer ) {
    if( capacity >= 0 ) {
      elementCount = 0;
      elementData = new HashMapEntry[ capacity == 0
                                                   ? 1
                                                   : capacity ];
   ...
   * given hash table, then adds all key/value pairs in the given hash table to
   * the new one, using the given element comparer.
   * 
   * @param capacity the maximum number of elements that can be added without
   *          rehashing
   * @param comparer the element comparer to use to compare keys and obtain hash
  ...
  /**
   * Returns the element comparer used to compare keys and to obtain hash codes
   * for keys, or <code>null</code> if no comparer has been provided.
   * 
  ...
  /**
   * Answers if this Hashtable contains the specified object as a key of one of
   * the key/value pairs.
   * 
  ...
  /**
   * Answers an Enumeration on the values of this Hashtable. The results of the
   ...
   * @return the value associated with the specified key, null if the specified
   *         key does not exist
   */
  ...
  /**
   * Associate the specified value with the specified key in this Hashtable. If
   * the key already exists, the old value is replaced. The key and value cannot
   * be null.
   * 
  ...
  public Object put( Object key, Object value ) {
    if( key != null && value != null ) {
      ...
      entry.key = key; // important to avoid hanging onto keys that are equal
                        // but "old" -- see bug 30607
      entry.value = value;
      return result;
    } else {
  ...
  /**
   * Increases the capacity of this Hashtable. This method is sent when the size
   * of this Hashtable exceeds the load factor.
   */
  private void rehash() {
