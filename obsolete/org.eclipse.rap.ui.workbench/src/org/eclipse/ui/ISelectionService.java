...
public interface ISelectionService {
   ...
   * changes in the part with the given id. This is independent of part
   * activation - the part need not be active for notification to be sent.
   * <p>
   ...
   * changed if the selection was triggered by the mouse but it has a delay if
   * the selection is triggered by the keyboard arrows. Has no effect if an
   * identical listener is already registered. Note: Works only for
   * StructuredViewer(s).
   * 
   * @param listener a selection listener
   */
   ...
   * not open, or if the selection in the active part is <em>undefined</em>
   * (the active part has no selection provider) the result will be
   * <code>null</code>.
   * 
  ...
  /**
   * Removes the given part-specific post selection listener. Has no effect if
   * an identical listener is not registered for the given part id.
   * 
   * @param partId the id of the part to track
   * @param listener a selection listener
   * @since 2.0
