...
//		activate(page, selection);
	...
	protected void openDialog(WorkbenchPage page) {
		...
		selection = null;
		final Shell dialog = new Shell(window.getShell(), SWT.NONE | SWT.APPLICATION_MODAL /*SWT.MODELESS */);
...
//				.getSite()).getPane().getControl().getMonitor().getBounds();
		// TODO: [bm] implement monitor support ;)
...
//		});
		...
		table.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				ok(dialog, table);
				activate(getWindow().getActivePage(), e.item.getData());
			}
