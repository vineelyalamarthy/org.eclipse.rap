...
//    folder.getControl().addDisposeListener( tabDisposeListener );
    ...
    for( Control current = contentProxy; current != folder.getControl()
      .getParent(); current = current.getParent() )
    {
  ...
  private void layoutContent() {
    if( current != null ) {
      ...
      Rectangle toControl = Geometry.toControl( folder.getControl().getParent(),
                                             clientArea );
      current.setBounds( toControl );
    }
  }
   ...
   * given index. Does nothing if a tab already exists for the given part. This
   * is intended to be called by TabOrder and its subclasses.
   * 
  ...
  private void childPropertyChanged( IPresentablePart part, int property ) {
    ...
    // If we're in the process of removing this part, it's possible that we
    // might still receive
    // some events for it. If everything is working perfectly, this should never
    // happen... however,
    ...
    switch( property ) {
      case IPresentablePart.PROP_HIGHLIGHT_IF_BACK:
        if( getCurrent() != part ) {// Set bold if it doesn't currently have
                                    // focus
          tab.setBold( true );
          initTab( tab, part );
        }
