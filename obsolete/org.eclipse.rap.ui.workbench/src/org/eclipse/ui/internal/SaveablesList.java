 ...
 * @since 3.2
 */
// TODO: could this be a session singleton?
public class SaveablesList implements ISaveablesLifecycleListener {
	...
	public Saveable[] getOpenModels() {
		...
		return (Saveable[]) allDistinctModels
				.toArray(new Saveable[allDistinctModels.size()]);
	}
	...
	private void logWarning(String message, Object source, Saveable model) {
		...
		AssertionFailedException assertionFailedException = new AssertionFailedException(
				"unknown saveable: " + model //$NON-NLS-1$
...
//				event.setVeto(true);
//			}
			// TODO: [bm] need veto? isVeto seems to be never called
			promptForSavingIfNecessary(PlatformUI
					.getWorkbench().getActiveWorkbenchWindow(), modelsClosing,
					modelsDecrementing, !event.isForce(), null);
			break;
	 ...
	 * @param listener
	 */
	public void removeModelLifecycleListener(
...
//			final IWorkbenchWindow window) {
	public void preCloseParts(List partsToClose, boolean save,
			final IWorkbenchWindow window, final IFunctionCallback callback) {
		// reference count (how many occurrences of a model will go away?)
		final PostCloseInfo postCloseInfo = new PostCloseInfo();
		for (Iterator it = partsToClose.iterator(); it.hasNext();) {
			...
			if (save && part instanceof ISaveablePart2) {
				...
				if (response == ISaveablePart2.CANCEL) {
					// user canceled
//					return null;
					callback.functionReturned(null);
					return;
		...
		if (save) {
			promptForSavingIfNecessary(window,
					postCloseInfo.modelsClosing,
					postCloseInfo.modelsDecrementing, true, new IFunctionCallback() {

						public void functionReturned(Object returnCode) {
							if(returnCode == Boolean.TRUE) {
								callback.functionReturned(null);
								return;
							}
							callback.functionReturned(postCloseInfo);
						}
...
// Set modelsClosing, Map modelsDecrementing, boolean canCancel) {
	private void promptForSavingIfNecessary(final IWorkbenchWindow window,
			final Set modelsClosing, Map modelsDecrementing,
			final boolean canCancel, final IFunctionCallback callback) {
		...
		for (Iterator it = modelsDecrementing.keySet().iterator(); it.hasNext();) {
			...
			if (modelDecrementing.isDirty()
					&& !modelsClosing.contains(modelDecrementing)) {
				modelsToOptionallySave.add(modelDecrementing);
			}
		}

		// TODO: [bm] callback hack - see original code
// boolean shouldCancel = modelsToOptionallySave.isEmpty() ? false :
// promptForSaving(modelsToOptionallySave,
// window, window, canCancel, true);

		if (!modelsToOptionallySave.isEmpty()) {
			promptForSaving(modelsToOptionallySave, window, window, canCancel,
					true, new IFunctionCallback() {

						public void functionReturned(Object returnCode) {

							if (returnCode == Boolean.TRUE) {
								callback.functionReturned(Boolean.TRUE);
								return;
							}
		...
		for (Iterator it = modelsClosing.iterator(); it
		.hasNext();) {
...
// window, window, canCancel, false);
		if (modelsToSave.isEmpty()) {
			callback.functionReturned(Boolean.FALSE);
			return;
		}
		promptForSaving(modelsToSave, window, window, canCancel, false,
				new IFunctionCallback() {

					public void functionReturned(Object returnCode) {
						callback.functionReturned(returnCode);
					}
	 ...
	 *            whether the models are referenced by open parts
	 * @param callback
	 *            callback object which gets the return code
	 */
...
// final boolean canCancel, boolean stillOpenElsewhere) {
	public void promptForSaving(final List modelsToSave,
			final IShellProvider shellProvider,
			final IRunnableContext runnableContext, final boolean canCancel,
			final boolean stillOpenElsewhere, final IFunctionCallback callback) {
		// Save parts, exit the method if cancel is pressed.
		if (modelsToSave.size() > 0) {
			boolean canceled = SaveableHelper
					.waitForBackgroundSaveJobs(modelsToSave);
			if (canceled) {
// return true;
				callback.functionReturned(Boolean.TRUE);
				return;
...
// !apiPreferenceStore.getBoolean(IWorkbenchPreferenceConstants.PROMPT_WHEN_SAVEABLE_STILL_OPEN);
			// TODO [bm] needs pref store
			boolean dontPrompt = stillOpenElsewhere && !true; // true is
																// default value
				...
				if (stillOpenElsewhere) {
					String message = NLS
							.bind(
									...
									model.getName());
//					MessageDialogWithToggle dialogWithToggle = new MessageDialogWithToggle(
...
//							buttons,
//							0,
//							WorkbenchMessages.EditorManager_closeWithoutPromptingOption,
//							false) {
...
//						}
//					};
					// TODO: [bm] MessageDialogWithToggle does not open - replacing it because
					// we also don't have a pref store
				...
				if (SaveableHelper.testGetAutomatedResponse() == SaveableHelper.USER_RESPONSE) {
					dialog.open(new IWindowCallback() {
						...
						public void windowClosed(int returnCode) {
							...
							if (stillOpenElsewhere) {
								// map value of choice back to ISaveablePart2
								// values
								switch (choice) {
...
// }
							...
							// The choice id is based on the order of button
							// labels
							// above.
							switch (choice) {
...
// return true;
							...
							callback.functionReturned(Boolean.valueOf(saveModels(modelsToSave,
									shellProvider, runnableContext)));
							return;
						}
				...
				// TODO [bm] needs ListSelectionDialog
...
// stillOpenElsewhere ?
// WorkbenchMessages.EditorManager_saveResourcesOptionallyMessage
...
// if (SaveableHelper.testGetAutomatedResponse() ==
// SaveableHelper.USER_RESPONSE) {
// int result = dlg.open();
...
// }
				throw new UnsupportedOperationException(
						"ListDialog not yet implemented");
				// Create save block.
...
// return saveModels(modelsToSave, shellProvider, runnableContext);
	 ...
	 */
	public boolean saveModels(final List finalModels,
			final IShellProvider shellProvider, IRunnableContext runnableContext) {
			...
			public void run(IProgressMonitor monitor) {
				...
				for (Iterator i = finalModels.iterator(); i.hasNext();) {
					...
					SaveableHelper.doSaveModel(model, new SubProgressMonitor(
							monitorWrap, 1), shellProvider, true);
...
// ILabelProvider labelProvider, String message, boolean canCancel, boolean
// stillOpenElsewhere) {
...
// }
	...
	public IWorkbenchPart[] getPartsForSaveable(Saveable model) {
		...
		for (Iterator it = modelMap.entrySet().iterator(); it.hasNext();) {
			...
			Set values = (Set) entry.getValue();
			if (values.contains(model)
					&& entry.getKey() instanceof IWorkbenchPart) {
				result.add(entry.getKey());
			}
		}
		return (IWorkbenchPart[]) result.toArray(new IWorkbenchPart[result
				.size()]);
	}
