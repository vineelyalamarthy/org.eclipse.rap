...
public interface ISizeProvider {
   ...
   * Integer.MAX_VALUE, ensuring that it is greater than any other integer.
   */
   ...
   * sizes. This is commonly used if the part wants to use a set of
   * predetermined sizes instead of using the workbench-provided size. For
   * example, computePreferredSize(horizontal, availableSpace, someWidth,
   * preferredSize) may return the nearest predetermined size. Note that this
   * flag should be used sparingly. It can prevent layout caching and cause the
   * workbench layout algorithm to degrade to exponential worst-case runtime. If
   * this flag is omitted, then computePreferredSize may be used to compute the
   * minimum and maximum sizes, but not for anything in between.</li>
   * </ul>
   * 
   ...
   * and the workbench's preferred size for the part. Parts can overload this to
   * enforce a minimum size, maximum size, or a quantized set of preferred
   ...
   *     </code>
   * getSizeFlags(boolean width) must return SWT.WRAP | SWT.MIN; </li>
   * </p>
   * 
   * @param width indicates whether a width (=true) or a height (=false) is
   *          being computed
   * @param availableParallel available space. This is a width (pixels) if width ==
   *          true, and a height (pixels) if width == false. A return value
   *          larger than this will be ignored.
   * @param availablePerpendicular available space perpendicular to the
   *          direction being measured or INFINITE if unbounded (pixels). This
   *          is a height if width == true, or a height if width == false.
   *          Implementations will generally ignore this argument unless they
   *          contain wrapping widgets. Note this argument will only contain
   *          meaningful information if the part returns the SWT.WRAP flag from
   *          getSizeFlags(width)
   * @param preferredResult preferred size of the control (pixels, <=
   *          availableParallel). Set to INFINITE if unknown or unbounded.
   ...
   *         value of INFINITE is permitted, indicating that the preferred size
   *         of the control is unbounded.
