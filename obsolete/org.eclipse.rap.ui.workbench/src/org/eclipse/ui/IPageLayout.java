 ...
 * <schtoo@schtoo.com> - Fix for 99155 - allow standalone view placeholders
 ******************************************************************************/
 ...
 * <strong>primaryId [':' secondaryId]</strong>. If a secondary id is given,
 * the view must allow multiple instances by having specified
...
public interface IPageLayout {
  ...
  /**
   * Adds an action set with the given id to this page layout. The id must name
   * an action set contributed to the workbench's extension point (named
   ...
   * compound ids. The primary id must name a view contributed to the
   ...
   * the UI to allow rapid navigation to appropriate new wizards. For example,
   * in the Eclipse IDE, these appear as items under the File > New menu. The id
  ...
  /**
   * Adds a perspective shortcut to the page layout. These are typically shown
   * in the UI to allow rapid navigation to appropriate new wizards. For
   * example, in the Eclipse IDE, these appear as items under the Window > Open
   * Perspective menu. The id must name a perspective extension contributed to
   * the workbench's perspectives extension point (named
   ...
   * placeholder. See the {@link IPageLayout} type documentation for more
   * details about compound ids. If the placeholder contains wildcards, it
   * remains in the layout, otherwise it is replaced by the view. If the primary
   * id of the placeholder has no wildcards, it must refer to a view contributed
   * to the workbench's view extension point (named
   * <code>"org.eclipse.ui.views"</code>).
   * 
   ...
   *          or <code>RIGHT</code>
   * @param ratio a ratio specifying how to divide the space currently occupied
   *          by the reference part, in the range <code>0.05f</code> to
   *          <code>0.95f</code>. Values outside this range will be clipped
   *          to facilitate direct manipulation. For a vertical split, the part
   *          on top gets the specified ratio of the current space and the part
   *          on bottom gets the rest. Likewise, for a horizontal split, the
   *          part at left gets the specified ratio of the current space and the
   *          part at right gets the rest.
  ...
  /**
   * Adds an item to the Show In prompter. The id must name a view contributed
   * to the workbench's view extension point (named
   * <code>"org.eclipse.ui.views"</code>).
   * 
   ...
   * The primary id must name a view contributed to the workbench's view
   * extension point (named <code>"org.eclipse.ui.views"</code>).
   * 
   ...
   * position and relative size of the folder is expressed relative to a
   * reference part.
   * 
   ...
   * identifier for the editor area is also stored in
  ...
  /**
   * Sets whether this layout is fixed. In a fixed layout, layout parts cannot
   * be moved or zoomed, and the initial set of views cannot be closed.
   * 
   * @param isFixed <code>true</code> if this layout is fixed,
  ...
  /**
   * Returns <code>true</code> if this layout is fixed, <code>false</code>
   * if not. In a fixed layout, layout parts cannot be moved or zoomed, and the
   * initial set of views cannot be closed. The default is <code>false</code>.
   * 
   * @return <code>true</code> if this layout is fixed, <code>false</code>
   *         if not.
   * @since 3.0
   */
  ...
  /**
   * Returns the layout for the view or placeholder with the given compound id
   * in this page layout. See the {@link IPageLayout} type documentation for
   * more details about compound ids. Returns <code>null</code> if the
   * specified view or placeholder is unknown to the layout.
   * 
   ...
   * 
   * @param viewId the compound view id
   * @param showTitle <code>true</code> to show the title and related
   *          controls, <code>false</code> to hide them
   * @param relationship the position relative to the reference part; one of
   *          <code>TOP</code>, <code>BOTTOM</code>, <code>LEFT</code>,
   *          or <code>RIGHT</code>
   * @param ratio a ratio specifying how to divide the space currently occupied
  ...
  /**
   * Adds a standalone view placeholder to this page layout. A view placeholder
   * is used to define the position of a view before the view appears.
   * Initially, it is invisible; however, if the user ever opens a view whose
   * compound id matches the placeholder, the view will appear at the same
   * location as the placeholder. See the {@link IPageLayout} type documentation
   * for more details about compound ids. If the placeholder contains wildcards,
   * it remains in the layout, otherwise it is replaced by the view. If the
   * primary id of the placeholder has no wildcards, it must refer to a view
   * contributed to the workbench's view extension point (named
   * <code>"org.eclipse.ui.views"</code>).
   * 
   * @param viewId the compound view id (wildcards allowed)
