...
//import org.eclipse.rap.jface.action.IStatusLineManager;
 ...
 * and override methods to configure a window's action bars to suit the needs of
...
public class ActionBarAdvisor {
   ...
   * bars of the corresponding workbench window; the remaining flags indicate
   ...
   * which combination of the menu bar, the tool bar, and the status line are to
   * be described. The actions included in the proxy action bars can be the same
   ...
   * calls <code>makeActions</code> if <code>FILL_PROXY</code> is specified,
   * then calls <code>fillMenuBar</code>, <code>fillCoolBar</code>, and
...
//      fillStatusLine( actionBarConfigurer.getStatusLineManager() );
   ...
   * to the list of actions to be disposed when the window is closed.
   * <p>
   ...
   * the <code>org.eclipse.ui.commands</code> extension point documentation
   * for more details.
   * </p>
   * 
  ...
  /**
   * Fills the status line with the main status line contributions for the
   * window.
   * <p>
   * The default implementation does nothing. Subclasses may override.
   * </p>
...
//  }
  ...
  /**
   * Restores arbitrary application-specific state information for this action
   * bar advisor.
   * <p>
   * The default implementation simply returns an OK status. Subclasses may
   * extend or override.
