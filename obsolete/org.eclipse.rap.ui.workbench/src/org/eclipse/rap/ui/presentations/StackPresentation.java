 ...
 ******************************************************************************/
package org.eclipse.rap.ui.presentations;
...
public abstract class StackPresentation implements ISizeProvider {
  ...
  /**
   * Activation state indicating that one of the parts in the presentation
   * currently has focus
   */
  public static final int AS_ACTIVE_FOCUS = 1;
  /**
   * Activation state indicating that none of the parts in the presentation have
   * focus, but one of the parts is being used as the context for global menus
   * and toolbars
   */
  ...
  /**
   * Returns the minimum size for this stack. The stack is prevented from being
   * resized smaller than this amount, and this is used as the default size for
   * the stack when it is minimized. Typically, this is the amount of space
   * required to fit the minimize, close, and maximize buttons and one tab.
   * 
  ...
  /**
   * Disposes all SWT resources being used by the stack. This is the last method
   * that will be invoked on the stack.
   */
  public abstract void dispose();

  /**
   * This is invoked to notify the presentation that its activation state has
   * changed. StackPresentations can have three possible activation states (see
   * the AS_* constants above)
   * 
  ...
  /**
   * This causes the presentation to become visible or invisible. When a
   * presentation is invisible, it must not respond to user input or modify its
   * parts. For example, a presentations will be made invisible if it belongs to
   * a perspective and the user switches to another perspective.
   * 
  ...
  /**
   * Sets the state of the presentation. That is, notifies the presentation that
   * is has been minimized, maximized, or restored. Note that this method is the
   * only way that a presentation is allowed to change its state.
   * <p>
   * If a presentation wishes to minimize itself, it must call setState on its
   * associated IStackPresentationSite. If the site chooses to respond to the
   * state change, it will call this method at the correct time. The
   * presentation should not call this method directly.
   * </p>
   * 
   ...
   * result of a drag/drop operation, it will be given a cookie identifying the
   * drop location. Has no effect if an identical part is already in the
   * presentation.
   * 
   * @param newPart the new part to add (not null)
   * @param cookie an identifier for a drop location, or null. When the
   *          presentation attaches a cookie to a StackDropResult, that cookie
   *          is passed back into addPart when a part is actually dropped in
   *          that location.
   */
  ...
  /**
   * Moves a part to a new location as the result of a drag/drop operation
   * within this presentation.
   * 
   * @param toMove a part that already belongs to this presentation
   * @param cookie a drop cookie returned by
   *          <code>StackPresentation#dragOver</code>
   * @since 3.1
   */
  ...
  /**
   * Brings the specified part to the foreground. This should not affect the
   * current focus.
   * 
   ...
   * @return a StackDropResult or null if the presentation does not have a drop
...
//                                            Point location );
   ...
   * 
   * @param context object that can be used to generate unique IDs for
   *          IPresentableParts (this may be a temporary object - the
   *          presentation should not keep any references to it)
...
//  }
   ...
   * 
   * @param context object that can be used to find IPresentableParts given
   *          string IDs (this may be a temporary object - the presentation
