...
public interface IStackPresentationSite {
   ...
   * @param initialPosition the mouse position at the time of the initial
   *          mousedown (display coordinates, not null)
   * @param keyboard true iff the drag was initiated via mouse dragging, and
   *          false if the drag may be using the keyboard
   */
   ...
   * 
   * @param toClose the set of parts to close (Not null. All of the entries must
   *          be non-null)
   */
  ...
  /**
   * Returns true iff the given part can be dragged. If this returns false, the
   * given part should not trigger a drag.
   * 
  ...
  /**
   * Adds system actions to the given menu manager. The site may make use of the
   * following group ids:
   * <ul>
   ...
   * </ul>
   * The presentation can control the insertion position by creating these group
   ...
   * changed. Hints to the workbench that it should trigger a layout at the next
   * opportunity.
   * 
   * @since 3.1
   */
