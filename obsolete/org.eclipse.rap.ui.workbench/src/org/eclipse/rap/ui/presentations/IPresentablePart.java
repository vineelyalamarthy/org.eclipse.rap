 ...
 * access to lifecycle events and allows repositioning of the part. Not intended
...
public interface IPresentablePart {
   ...
   * A part is visible iff any part of its widgetry can be seen.
   * 
   * @param isVisible true if the part has just become visible, false if the
   *          part has just become hidden
   */
   ...
   * none. This is a substring of the part's title. A typical title will consist
   * of the part name, a separator, and a status message describing the current
   * contents.
   * <p>
   * Presentations can query getName() and getTitleStatus() if they want to
   * display the status message and name separately, or they can use getTitle()
   * if they want to display the entire title.
   * </p>
   * 
   ...
   * editors, this indicates that the part has changed since the last save. For
   * views, this indicates that the view contains interesting changes that it
   * wants to draw the user's attention to.
   * 
  ...
  /**
   * Returns the local toolbar for this part, or null if this part does not have
   * a local toolbar. Callers must not dispose or downcast the return value.
   * 
  ...
  /**
   * Returns an SWT control that can be used to indicate the tab order for this
   * part. This can be returned as part of the result to
   * {@link StackPresentation#getTabList(IPresentablePart)}. Any other use of
   * this control is unsupported. This may return a placeholder control that is
   * only meaningful in the context of <code>getTabList</code>.
   * 
