...
import org.eclipse.rap.ui.internal.layout.SizeCache;
 ...
 * bounds of the dummy control also changes the bounds of the target. This
 * allows any Composite to lay out a control that isn't one of its children.
 * <p>
 * For example, imagine you have a ViewForm and a ToolBar that share the same
 * parent and you want the ToolBar to be located in the upper-right corner of
 * the ViewForm. If the ToolBar were a child of the ViewForm, this could be done
 * easily by calling viewForm.setTopRight(toolBar). However, this is impossible
 * since ViewForm.setTopRight will only accept a child control. Instead, we
 * create a ProxyControl as a child of the viewForm, and set the toolbar as its
 * target. The ViewForm will treat the ProxyControl just like any other child,
 * but it will actually be arranging the ToolBar.
 * </p>
 * <p>
 * For example:
 * </p>
 ...
 * </code>
 * <p>
 * This is intended to simplify management of view toolbars in the presentation
 * API. Presentation objects have no control over where the view toolbars are
 * created in the widget hierarchy, but they may wish to control the position of
 * the view toolbars using traditional SWT layouts and composites.
 * </p>
 */
public class ProxyControl {
  ...
  /**
   * Visibility state of the proxy control the last time it had a non-null
   * target. Note: when the target is set to null, we force the proxy to become
   * invisible and use this variable to remember the initial state when we get a
   * new non-null target.
   */
  private boolean visible = true;
  /**
   * Dispose listener. Breaks the link between the target and the proxy if
   * either control is disposed.
   */
...
//  };
  ...
  /**
   * Allow the visibility of the proxy control to be updated. When the target is
   * not <code>null</code> it's visibility is tied to the listener. But in the
   * case where some action causes the target to be populated while its
   * visibility is <code>false</code>, it won't re-appear until its
   * visibility is set to <code>true</code>.
   * 
   * @param visible <code>true</code> - set it to visible
   * @since 3.2
   */
  ...
  /**
   * Movement listener. Updates the bounds of the target to match the bounds of
   * the dummy control.
   */
  ...
  /**
   * Creates a new ProxyControl as a child of the given parent. This is an
   * invisible dummy control. If given a target, the ProxyControl will update
   * the bounds of the target to match the bounds of the dummy control.
   * 
...
//    control = new Composite( parent, SWT.NO_BACKGROUND );
    control = new Composite( parent, RWT.NONE );
    control.setVisible( false );
    // Attach a layout to the dummy composite. This is used to make the
    // preferred
    ...
    control.setLayout( new Layout() {
      ...
      protected void layout( Composite composite, boolean flushCache ) {
        ProxyControl.this.layout();
        // does nothing. The bounds of the target are updated by the
        // controlListener
      }
                                   ...
                                   boolean flushCache )
      {
        if( targetCache == null ) {
          ...
          // Note: If we returned (0,0), SWT would ignore the result and use a
          // default value.
...
//        // If we had previously forced the dummy control invisible, restore its
//        // visibility
...
//        }
//        // Make the new target's visiblity match the visibility of the dummy
//        // control
...
//    Rectangle displayBounds = Geometry.toDisplay( control.getParent(),
//                                                  control.getBounds() );
//    // Clip the bounds of the target so that it doesn't go outside the dummy
//    // control's parent
...
//    // Compute the bounds of the target, in the local coordinate system of its
//    // parent
//    Rectangle targetBounds = Geometry.toControl( getTarget().getParent(),
//                                                 displayBounds );
//    // Move the target
