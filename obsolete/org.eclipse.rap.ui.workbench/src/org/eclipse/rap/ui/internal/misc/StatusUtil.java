...
public class StatusUtil {
  ...
  public static IStatus newStatus( IStatus originalStatus, String newMessage ) {
    return new Status( originalStatus.getSeverity(),
                       originalStatus.getPlugin(),
                       originalStatus.getCode(),
                       newMessage,
  ...
  public static Throwable getCause( Throwable exception ) {
    ...
    if( exception != null ) {
      if( exception instanceof CoreException ) {
        // Workaround: CoreException contains a cause, but does not actually
        // implement getCause().
        // If we get a CoreException, we need to manually unpack the cause.
        // Otherwise, use
        // the general-purpose mechanism. Remove this branch if CoreException
        // ever implements
        ...
        try {
          Method causeMethod = exception.getClass()
            .getMethod( "getCause", new Class[ 0 ] ); //$NON-NLS-1$
