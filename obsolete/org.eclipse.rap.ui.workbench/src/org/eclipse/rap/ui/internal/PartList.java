...
import org.eclipse.rap.ui.IEditorPart;
...
public abstract class PartList {
  ...
  public IEditorPart getActiveEditor() {
    return activeEditorReference == null
                                        ? null
                                        : activeEditorReference.getEditor( false );
  }

  public IWorkbenchPart getActivePart() {
    return activePartReference == null
                                      ? null
...
//        IWorkbenchPartSite site = ( ( MultiEditor )part ).getActiveEditor()
//          .getSite();
...
//    modelManager.postOpen( actualPart );
  ...
  /**
   * Called when a concrete part is about to be destroyed. This is called BEFORE
   * disposal happens, so the part should still be accessable from the part
   * reference.
   * 
   * @param ref
   */
  ...
  /**
   * Fire the event indicating that a part reference was just realized. That is,
   * the concrete IWorkbenchPart has been attached to the part reference.
   * 
   ...
   * @param part active part reference or null if none
   */
  protected abstract void fireActivePartChanged( IWorkbenchPartReference oldPart,
                                                 IWorkbenchPartReference newPart );
