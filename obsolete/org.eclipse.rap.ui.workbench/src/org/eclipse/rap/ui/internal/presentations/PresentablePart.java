...
import org.eclipse.rap.ui.presentations.IPartMenu;
import org.eclipse.rap.ui.presentations.IPresentablePart;

/**
 * This is a lightweight adapter that allows PartPanes to be used by a
 * StackPresentation. All methods either redirect directly to PartPane or do
 * trivial type conversions. All listeners registered by the presentation are
 * kept here rather than registering them directly on the PartPane. This allows
 * us to remove all listeners registered by a presentation that has been
 * disposed, offering some protection against memory leaks.
 */
public class PresentablePart implements IPresentablePart {

  private PartPane part;
  /**
   * Local listener list -- we use this rather than registering listeners
   * directly on the part in order to protect against memory leaks in badly
   * behaved presentations.
   */
...
//  private IPropertyListener lazyPropertyListenerProxy;
  ...
  // True iff the "get" methods are returning up-to-date info from the real part
  // (disabled
...
//      if( part.getPage().getActivePart() == part.getPartReference()
//        .getPart( false ) )
//      {
//        part.setFocus();
//      } else {
