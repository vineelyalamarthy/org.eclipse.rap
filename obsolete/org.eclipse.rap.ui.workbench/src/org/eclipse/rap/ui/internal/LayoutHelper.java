...
import org.eclipse.rap.ui.PartInitException;
 ...
 * and <code>FolderLayout</code>) utilize for activities support and view
 * creation.
 * 
 * @since 3.0
 */
class LayoutHelper {
   ...
   * layout is the currently active perspective in its window, then activate the
   * views immediately.
   * <li> if the identifier becomes enabled and the perspective of the page
...
//          IIdentifier thisIdentifier = identifierEvent.getIdentifier();
//          if( thisIdentifier.isEnabled() ) {
//            // show view
//            thisIdentifier.removeIdentifierListener( this );
//            IWorkbenchPage activePage = partPage.getWorkbenchWindow()
//              .getActivePage();
...
//  }
  ...
  {
    String primary = ViewFactory.extractPrimaryId( viewId );
    String secondary = ViewFactory.extractSecondaryId( viewId );
    WorkbenchPartReference ref 
      = ( WorkbenchPartReference )factory.createView( primary, secondary );
