...
public interface ITrimManager {
   ...
   * Trim area location.
   */
  public static final int TOP = RWT.TOP;
  /**
   * Trim area location.
   */
  public static final int BOTTOM = RWT.BOTTOM;
  /**
   * Trim area location.
   */
  public static final int LEFT = RWT.LEFT;
  /**
   * Trim area location.
   */
  public static final int RIGHT = RWT.RIGHT;
  /**
   * Trim area location.
   */
  public static final int NONTRIM = RWT.DEFAULT;
   ...
   * for every trim control. If the given widget is already a trim widget, it
   * will be moved to the new position. Specifying a position allows a new
   * widget to be inserted between existing trim widgets.
   * <p>
   * For example, this method allows the caller to say "insert this new control
   * as trim along the bottom of the layout, to the left of this existing
   * control".
   * </p>
   * 
   * @param trim new window trim to be added
   * @param areaId the area ID
   * @param beforeMe trim to insert before, <code>null</code> to insert at the
   *          end
   * @see #getAreaIds()
   */
  ...
  /**
   * Removes the given window trim. Note that this has no effect if window trim
   * is not our window trim.
   * 
   ...
   *         currently support RWT.TOP, RWT.BOTTOM, RWT.LEFT, and RWT.RIGHT.
   * @since 3.2
   */
  public int[] getAreaIds();

  /**
   * Return a copy of the IWindowTrim in an ordered array. This will not return
   * <code>null</code>. This array can be used to shuffle items around in
   * {@link #updateAreaTrim(int, List, boolean) }.
   * 
   * @param areaId the trim area id
   ...
   * applies the IWindowTrim contains in the array to the trim area named "ID".
   * 
   * @param id the trim area ID
   * @param trim the trim array must not be <code>null</code>.
   * @param removeExtra if <code>true</code> the any trim in the specified
   *          trim area that's not contained in the List is removed from the
   *          window trim (but not disposed()). If <code>false</code> then the
  ...
  /**
   * Update the visibility of the trim controls. It updates any docking handles
   * as well. It has no effect on visiblity if the window trim doesn't belong to
   * this TrimLayout.
   * 
