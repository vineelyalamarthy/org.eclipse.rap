 ...
 * ViewPane and EditorPane, and make this class non-abstract. TODO: Stop
 * subclassing LayoutPart. This class cannot be interchanged with other
 * LayoutParts. Pointers that refer to PartPane instances should do so directly
...
//      // Hack: Currently, SWT sets focus whenever we call Control.traverse. This
//      // doesn't
//      // cause too much of a problem for ctrl-pgup and ctrl-pgdn, but it is
//      // seriously unexpected
//      // for other traversal events. When (and if) it becomes possible to call
//      // traverse() without
//      // forcing a focus change, this if statement should be removed and ALL
//      // events should be
...
//  };
  ...
  public void addSizeMenuItem( Menu menu, int index ) {
    // Add size menu
//    MenuItem item = new MenuItem( menu, RWT.CASCADE, index );
...
//    control.addListener( RWT.Activate, this );
...
//      control.removeListener( RWT.Activate, this );
...
//    partReference.removePropertyListener( this );
  ...
  /**
   * User has requested to close the pane. Take appropriate action depending on
   * type.
   */
...
//  }
  ...
  public void requestActivation() {
    ...
    // Cannot activate the outer bit of a MultiEditor. In previous versions of
    // the
    // workbench, MultiEditors had their own implementation of EditorPane for
    // the purpose
    // of overriding requestActivation with a NOP... however, keeping the old
    // pattern would
    // mean it is necessary to eagerly activate an editor's plugin in order to
    // determine
...
//    container.findSashes( this, result );
  ...
  /**
   * Enable the user to resize this part using the keyboard to move the
   * specified sash
   */
...
//    sash.setFocus();
   ...
   */
  protected void addSizeItem( Menu sizeMenu,
                              String labelMessage,
                              final Sash sash )
  {
    MenuItem item = new MenuItem( sizeMenu, RWT.NONE );
...
//    addSizeItem( sizeMenu, WorkbenchMessages.PartPane_sizeTop, sashes.top );
//    addSizeItem( sizeMenu, WorkbenchMessages.PartPane_sizeBottom, sashes.bottom );
  ...
  /**
   * Show a highlight for the receiver if it is not currently the part in the
   * front of its presentation.
   */
   ...
   * tests. Although these are english readable strings, they do not need to be
   * translated.
   * 
   * @param buf
   */
...
//  }
  ...
  /**
   * Clears all contribution items from the contribution managers (this is done
   * separately from dispose() since it is done after the part is disposed).
   * This is a bit of a hack. Really, the contribution managers should be part
   * of the site, not the PartPane. If these were moved elsewhere, then disposal
