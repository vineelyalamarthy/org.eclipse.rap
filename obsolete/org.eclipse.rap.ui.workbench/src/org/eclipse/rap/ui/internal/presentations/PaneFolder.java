...
import org.eclipse.rap.rwt.custom.CTabFolder2Adapter;
...
import org.eclipse.rap.ui.internal.presentations.util.ProxyControl;
...
//      }
  ...
  /**
   * State of the folder at the last mousedown event. This is used to prevent a
   * mouseup over the minimize or maximize buttons from undoing a state change
...
//      titleAreaProxy = new Composite( tabFolder, SWT.NO_BACKGROUND );
      titleAreaProxy = new Composite( tabFolder, RWT.NONE );
      ...
      tabFolder.setTopRight( titleAreaProxy, RWT.FILL );
...
//      viewForm = new ViewForm( tabFolder, SWT.NO_BACKGROUND );
      viewForm = new ViewForm( tabFolder, RWT.NONE );
  ...
  /**
   * Returns the title area (the empty region to the right of the tabs), in the
   * tab folder's coordinate system.
   * 
...
//    } );
  ...
  /**
   * Sets the top-center control (usually a toolbar), or null if none. Note that
   * the control can have any parent.
   * 
...
//      viewForm.setLayoutDeferred( true );
      ...
      Point topRightSize = topRightCache.computeSize( RWT.DEFAULT, RWT.DEFAULT );
      Point topCenterSize = topCenterCache.computeSize( RWT.DEFAULT,
                                                        RWT.DEFAULT );
      ...
      if( putTrimOnTop ) {
        ...
        Rectangle topRightArea = new Rectangle( titleArea.x
                                                    ...
                                                    - topRightSize.x,
                                                titleArea.y
                                                    + ( titleArea.height - topRightSize.y )
                                                    / 2,
                                                topRightSize.x,
                                                topRightSize.y );
        if( topRight != null ) {
        ...
        if( topCenter != null ) {
          Rectangle topCenterArea = new Rectangle( topRightArea.x
                                                       - topCenterSize.x,
                                                   titleArea.y
                                                       + ( titleArea.height - topCenterSize.y )
                                                       / 2,
                                                   topCenterSize.x,
                                                   topCenterSize.y );
          Rectangle localCoords = Geometry.toControl( topCenter.getParent(),
                                                      topCenterArea );
...
//    tabFolder.setSelectionForeground( fgColor );
   ...
   * @param vertical
   */
  public void setSelectionBackground( Color[] bgColors,
                                      int[] percentages,
                                      boolean vertical )
  {
...
//
   ...
   * 
   * @param count The number of characters to display in the tab folder; this
...
//  }
  ...
  /**
   * Propogate the visibility change requests to the proxy controls. When their
   * target is null, they no longer get visibility updates. Currently this only
   * propagates the changes to the ProxyControls held by this folder.
   * 
   * @param visible <code>true</code> - it's visible.
   * @since 3.2
   */
