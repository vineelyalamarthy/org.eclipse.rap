                           ...
                           Object target )
  {
    ...
    if( helpContextId != null ) {
      ...
      if( helpContextId.indexOf( "." ) == -1 ) { //$NON-NLS-1$
        ...
        // qualified)
        fullID = actionElement.getNamespace() + "." + helpContextId;//$NON-NLS-1$
      }
//      PlatformUI.getWorkbench().getHelpSystem().setHelp( action, fullID );
    ...
    if( style != null ) {
      // Since 2.1, the "state" and "pulldown" attributes means something
      // different
    ...
    String extendingPluginId = actionElement.getDeclaringExtension()
      .getNamespace();
    if( icon != null ) {
      action.setImageDescriptor( AbstractUIPlugin.imageDescriptorFromPlugin( extendingPluginId,
                                                                             icon ) );
    }
    if( hoverIcon != null ) {
//      action.setHoverImageDescriptor( AbstractUIPlugin.imageDescriptorFromPlugin( extendingPluginId,
//                                                                                  hoverIcon ) );
    }
    if( disabledIcon != null ) {
//      action.setDisabledImageDescriptor( AbstractUIPlugin.imageDescriptorFromPlugin( extendingPluginId,
//                                                                                     disabledIcon ) );
    }
    if( accelerator != null ) {
                                     ...
                                     String style )
  {
    ...
    switch( targetType ) {
      case T_VIEW:
        return new ViewPluginAction( actionElement,
                                     ( IViewPart )target,
                                     id,
                                     actionStyle );
      case T_EDITOR:
        throw new UnsupportedOperationException();
//        return new EditorPluginAction( actionElement,
//                                       ( IEditorPart )target,
//                                       id,
//                                       actionStyle );
      case T_WORKBENCH:
...
//        return new ObjectPluginAction( actionElement, id, actionStyle );
  ...
  /**
   * Returns menu path where this action should be added. If null, the action
   * will not be added into the menu.
   * 
  ...
  /**
   * Returns id of the tool bar where this action should be added. If null,
   * action will not be added to the tool bar.
   * 
