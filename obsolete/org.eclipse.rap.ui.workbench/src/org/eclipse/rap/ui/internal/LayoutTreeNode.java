...
/**
 * Implementation of a tree node. The node represents a sash and it allways has
 * two children.
 */
public class LayoutTreeNode extends LayoutTree {
  ...
  public LayoutPart computeRelation( ArrayList relations ) {
    ...
    r.relationship = getSash().isVertical()
                                           ? IPageLayout.RIGHT
                                           : IPageLayout.BOTTOM;
    relations.add( 0, r );
    return r.relative;
  ...
  /**
   * Go up in the tree finding a parent that is common of both children. Return
   * the subtree.
   */
   ...
   * the subtree.
   */
  LayoutTreeNode findCommonParent( LayoutPart child1,
                                   LayoutPart child2,
                                   boolean foundChild1,
                                   boolean foundChild2 )
  {
  ...
  /**
   * Sets the elements in the array of sashes with the Left,Rigth,Top and Botton
...
//  }
  ...
  /**
   * Go up from the subtree and return true if all the sash are in the direction
   * specified by <code>isVertical</code>
   */
  ...
  public int doComputePreferredSize( boolean width,
                                     ...
                                     int preferredParallel )
  {
    ...
    if( !children[ 0 ].isVisible() ) {
      return children[ 1 ].computePreferredSize( width,
                                                 availableParallel,
                                                 availablePerpendicular,
                                                 preferredParallel );
    }
    if( !children[ 1 ].isVisible() ) {
      return children[ 0 ].computePreferredSize( width,
                                                 availableParallel,
                                                 availablePerpendicular,
                                                 preferredParallel );
    }
    ...
    if( width == getSash().isVertical() ) {
      ...
      // Computing the dimension parallel to the sash. We will compute and
      // return the preferred size
      // of whichever child is closest to the ideal size.
      ChildSizes sizes;
      // First compute the dimension of the child sizes perpendicular to the
      // sash
      sizes = computeChildSizes( availablePerpendicular,
                                 availableParallel,
                                 getSash().getLeft(),
                                 getSash().getRight(),
                                 availablePerpendicular );
      // Use this information to compute the dimension of the child sizes
      // parallel to the sash.
      // Return the preferred size of whichever child is largest
      int leftSize = children[ 0 ].computePreferredSize( width,
                                                         availableParallel,
                                                         sizes.left,
                                                         preferredParallel );
      // Compute the preferred size of the right child
      int rightSize = children[ 1 ].computePreferredSize( width,
                                                          availableParallel,
                                                          sizes.right,
                                                          preferredParallel );
  ...
  /**
   * Computes the pixel sizes of this node's children, given the available space
   * for this node. Note that "width" and "height" actually refer to the
   * distance perpendicular and parallel to the sash respectively. That is,
   * their meaning is reversed when computing a horizontal sash.
   * 
   * @param width the pixel width of a vertical node, or the pixel height of a
   *          horizontal node (INFINITE if unbounded)
   * @param height the pixel height of a vertical node, or the pixel width of a
   *          horizontal node (INFINITE if unbounded)
   * @return a struct describing the pixel sizes of the left and right children
   *         (this is a width for horizontal nodes and a height for vertical
   *         nodes)
   */
  ChildSizes computeChildSizes( int width,
                                int height,
                                int left,
                                int right,
                                int preferredWidth )
  {
    ...
    if( width == INFINITE ) {
      if( preferredWidth == INFINITE ) {
        return new ChildSizes( children[ 0 ].computeMaximumSize( vertical,
                                                                 height ),
                               children[ 1 ].computeMaximumSize( vertical,
                                                                 height ),
                               false );
      }
      if( preferredWidth == 0 ) {
        return new ChildSizes( children[ 0 ].computeMinimumSize( vertical,
                                                                 height ),
                               children[ 1 ].computeMinimumSize( vertical,
                                                                 height ),
                               false );
      }
    }
    ...
    // Subtract the SASH_WIDTH from preferredWidth and width. From here on,
    // we'll deal with the
    ...
    // Keep track of the available space for each child, given the minimum size
    // of the other child
    int leftAvailable = Math.min( leftMaximum,
                                  Math.max( 0, subtract( width, rightMinimum ) ) );
    int rightAvailable = Math.min( rightMaximum,
                                   Math.max( 0, subtract( width, leftMinimum ) ) );
    // Figure out the ideal size of the left child
    int idealLeft = Math.max( leftMinimum,
                              Math.min( preferredWidth,
                                        left
                                            + ( int )Math.round( redistribute
                                                                 * wLeft
                                                                 / wTotal ) ) );
    // If the right child can't use all its available space, let the left child
    // fill it in
    ...
    idealLeft = children[ 0 ].computePreferredSize( vertical,
                                                    leftAvailable,
                                                    height,
                                                    idealLeft );
    ...
    rightAvailable = Math.max( 0, Math.min( rightAvailable,
                                            subtract( width, idealLeft ) ) );
    idealRight = Math.min( idealRight, rightAvailable );
    idealRight = children[ 1 ].computePreferredSize( vertical,
                                                     rightAvailable,
                                                     height,
                                                     idealRight );
    idealRight = Math.max( idealRight, rightMinimum );
    return new ChildSizes( idealLeft,
                           idealRight,
  ...
  protected int doGetSizeFlags( boolean width ) {
    ...
    return ( ( leftFlags | rightFlags ) & ~RWT.MAX )
           | ( leftFlags & rightFlags & RWT.MAX );
  }
  ...
  public void doSetBounds( Rectangle bounds ) {
    ...
    ChildSizes childSizes = computeChildSizes( bounds.width,
                                               bounds.height,
                                               getSash().getLeft(),
                                               getSash().getRight(),
                                               bounds.width );
    ...
    Rectangle leftBounds = new Rectangle( bounds.x,
                                          bounds.y,
                                          childSizes.left,
                                          bounds.height );
    Rectangle sashBounds = new Rectangle( leftBounds.x + leftBounds.width,
                                          bounds.y,
                                          SASH_WIDTH,
                                          bounds.height );
    Rectangle rightBounds = new Rectangle( sashBounds.x + sashBounds.width,
                                           bounds.y,
                                           childSizes.right,
                                           bounds.height );
    if( !vertical ) {
