...
import org.eclipse.rap.ui.internal.misc.StatusUtil;
import org.eclipse.rap.ui.internal.registry.IWorkbenchRegistryConstants;
import org.eclipse.rap.ui.internal.util.BundleUtility;

/**
 * A PluginAction is a proxy for an action extension. At startup we read the
 * registry and create a PluginAction for each action extension. This plugin
 * action looks like the real action ( label, icon, etc ) and acts as a proxy
 * for the action until invoked. At that point the proxy will instantiate the
 * real action and delegate the run method to the real action. This makes it
 * possible to load the action extension lazily. Occasionally the class will ask
 * if it is OK to load the delegate (on selection changes). If the plugin
 * containing the action extension has been loaded then the action extension
 * itself will be instantiated.
 */
public abstract class PluginAction extends Action
  implements ISelectionListener, ISelectionChangedListener,
  INullSelectionListener, IPluginContribution
{
    ...
    if( delegate == null && runAttribute != null ) {
      try {
        ...
        if( e instanceof CoreException ) {
          ...
          status = StatusUtil.newStatus( IStatus.ERROR,
  ...
  /**
   * Returns true if the declaring plugin has been loaded and there is no need
   * to delay creating the delegate any more.
   */
...
//                                       WorkbenchMessages.PluginAction_disabledMessage );
  ...
  /**
   * Handles selection change. If rule-based enabled is defined, it will be
   * first to call it. If the delegate is loaded, it will also be given a
   * chance.
   * 
