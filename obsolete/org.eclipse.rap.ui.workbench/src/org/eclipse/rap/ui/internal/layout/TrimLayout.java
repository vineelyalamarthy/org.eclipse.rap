...
import org.eclipse.rap.ui.internal.TrimDragPreferences;
 ...
 * <p>
 * Unlike most other RWT layouts, this layout does not require layout data to be
...
public class TrimLayout extends Layout implements ICachingLayout, ITrimManager {
   ...
   */
  private static final int[] TRIM_ID_INFO = {
...
//    trimLocked = store.getBoolean( IWorkbenchPreferenceConstants.LOCK_TRIM );
    // TODO:[fappel] preference store implementation...
    trimLocked = false;
    createTrimArea( TOP_ID, TOP_ID.toString(), RWT.DEFAULT, RWT.TOP );
    createTrimArea( BOTTOM_ID, BOTTOM_ID.toString(), RWT.DEFAULT, RWT.BOTTOM );
    createTrimArea( LEFT_ID, LEFT_ID.toString(), RWT.DEFAULT, RWT.LEFT );
    createTrimArea( RIGHT_ID, RIGHT_ID.toString(), RWT.DEFAULT, RWT.RIGHT );
  }

  private void createTrimArea( Integer id,
                               String displayName,
                               int trimSize,
                               int trimMods )
  {
   ...
   * RWT.DEFAULT, then the trim size will be computed from child controls.
   * 
   ...
   * RWT.LEFT if the control is docked on the left, RWT.RIGHT if docked on the
   * right, etc. Returns RWT.DEFAULT if the given control is not a trim control.
   * 
   * @param trimControl control to query
   * @return The area ID of this control. If the control is not part of our
   *         trim, return RWT.DEFAULT.
   * @see #getAreaIds()
   */
  ...
  /**
   * @param control new window trim to be added
   * @param areaId the area ID
   * @see #getAreaIds()
   * @deprecated
   */
  ...
  /**
   * @param trim new window trim to be added
   * @param areaId the area ID
   * @param beforeMe if null, the control will be inserted as the last trim
   *          widget on this side of the layout. Otherwise, the control will be
  ...
  public void addTrim( int areaId, IWindowTrim trim, IWindowTrim beforeMe ) {
    ...
    if( !trimLocked && trim.getValidSides() != RWT.NONE ) {
...
//        }
    ...
    if( beforeMe != null ) {
      TrimDescriptor beforeDesc = ( TrimDescriptor )fTrimDescriptors.get( beforeMe.getId() );
  ...
  /**
   * Force a layout of the trim...we hack this by calling the LayoutUtil with
   * the first piece of trim that we find...(kludge!!)
   */
  ...
  public void removeTrim( IWindowTrim toRemove ) {
    TrimDescriptor desc = ( TrimDescriptor )fTrimDescriptors.remove( toRemove.getId() );
    if( desc == null ) {
      return;
    }
                               ...
                               boolean flushCache )
  {
    ...
    int horizontalTrim = left.calculateTrimSize( wHint, hHint )
                         + right.calculateTrimSize( wHint, hHint )
                         + ( 2 * marginWidth )
                         + leftSpacing
                         + rightSpacing;
    int verticalTrim = top.calculateTrimSize( wHint, hHint )
                       + bottom.calculateTrimSize( wHint, hHint )
                       + ( 2 * marginHeight )
                       + topSpacing
                       + bottomSpacing;
    Point innerSize = centerArea.computeSize( wHint == RWT.DEFAULT
                                                                  ? wHint
                                                                  : wHint
                                                                    - horizontalTrim,
                                              hHint == RWT.DEFAULT
  ...
  protected void layout( Composite composite, boolean flushCache ) {
    ...
    int widthOfCenterPane = clientArea.width
                            - trim_left
                            - trim_right
                            - leftSpacing
                            - rightSpacing;
    ...
    // handle the CBanner's CoolBar wrapping. The code that calculates
    // 'trim_top'
    ...
    trim_top = arrange( new Rectangle( leftOfLayout,
                                       topOfLayout,
                                       clientArea.width,
                                       trim_top ),
                        top.getCaches(),
    ...
    int heightOfCenterPane = clientArea.height
                             - trim_top
                             - trim_bottom
                             - topSpacing
                             - bottomSpacing;
    int bottomOfCenterPane = clientArea.y + clientArea.height - trim_bottom;
    arrange( new Rectangle( leftOfCenterPane,
                            bottomOfCenterPane,
                            widthOfCenterPane,
                            trim_bottom ),
             bottom.getCaches(),
             !bottom.isVertical(),
             spacing );
    arrange( new Rectangle( leftOfLayout,
                            topOfCenterPane,
                            trim_left,
                            clientArea.height - trim_top ),
             left.getCaches(),
             !left.isVertical(),
             spacing );
    arrange( new Rectangle( rightOfCenterPane,
                            topOfCenterPane,
                            trim_right,
                            clientArea.height - trim_top ),
             right.getCaches(),
   ...
   * @param caches a list of SizeCaches for controls that will span the
   *          rectangle
   * @param horizontally how we are filling the rectangle
   * @param spacing in pixels
   */
  private static int arrange( Rectangle area,
                              List caches,
                              ...
                              int spacing )
  {
    ...
    int available = Geometry.getDimension( area, horizontally )
                    - used
                    - spacing
                    * ( caches.size() - 1 );
    idx = 0;
    ...
    while( iter.hasNext() ) {
      ...
      if( next.getControl().isVisible() ) {
        ...
        if( TrimDragPreferences.showRaggedTrim() ) {
          Point prefSize = next.computeSize( RWT.DEFAULT, RWT.DEFAULT );
          if( horizontally ) {
            // HACK!! Surgical Fix: Ideally the fix would handle 'wrapping' trim
            // on any side. The following code is in place specifically to
   ...
   * this will be a composite that contains the main widgetry of the
   * application.
   * 
   * @param center control that will occupy the center of the layout, or null if
   *          none
   */
   ...
   * 
   * @param window the window that has the trim
   * @param areaId the side it's on
   ...
   * 
   * @param trim the Control to find.
