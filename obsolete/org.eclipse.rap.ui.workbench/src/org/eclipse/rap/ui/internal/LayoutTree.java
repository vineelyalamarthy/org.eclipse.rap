 ...
 * IBM Corporation - initial API and implementation Randy Hudson
 * <hudsonr@us.ibm.com> - Fix for bug 19524 - Resizing WorkbenchWindow resizes
 * Views Cagatay Kavukcuoglu <cagatayk@acm.org> - Fix for bug 10025 - Resizing
 * views should not use height ratios
 ******************************************************************************/
...
/**
 * Implementation of a tree where the node is allways a sash and it allways has
 * two chidren. If a children is removed the sash, ie the node, is removed as
 * well and its other children placed on its parent.
 */
public class LayoutTree implements ISizeProvider {
  ...
  LayoutPart part;
  // Cached information
  private int cachedMinimumWidthHint = RWT.DEFAULT;
  private int cachedMinimumWidth = RWT.DEFAULT;
  private int cachedMinimumHeightHint = RWT.DEFAULT;
  private int cachedMinimumHeight = RWT.DEFAULT;
  private int cachedMaximumWidthHint = RWT.DEFAULT;
  private int cachedMaximumWidth = RWT.DEFAULT;
  private int cachedMaximumHeightHint = RWT.DEFAULT;
  private int cachedMaximumHeight = RWT.DEFAULT;
  // Cached size flags
  ...
  /**
   * Add the relation ship between the children in the list and returns the left
   * children.
   */
  ...
  /**
   * Find a LayoutPart in the tree and return its sub-tree. Returns null if the
   * child is not found.
   */
...
//  }
  ...
  /**
   * Find a sash in the tree and return its sub-tree. Returns null if the sash
   * is not found.
   */
  ...
  /**
   * Return the bounds of this tree which is the rectangle that contains all
  ...
  /**
   * Subtracts two integers. If a is INFINITE, this is treated as positive
   * infinity.
   * 
   ...
   * to INFINITE. Many of the methods of this class use positive integers as
   * sizes, with INFINITE indicating positive infinity. This picks up accidental
   * addition or subtraction from infinity.
   * 
   * @param toCheck integer to validate
   * @since 3.1
   */
  public static void assertValidSize( int toCheck ) {
    Assert.isTrue( toCheck >= 0
                   && ( toCheck == INFINITE || toCheck < INFINITE / 2 ) );
  }

  /**
   * Computes the preferred size for this object. The interpretation of the
   * result depends on the flags returned by getSizeFlags(). If the caller is
   * looking for a maximum or minimum size, this delegates to computeMinimumSize
   * or computeMaximumSize in order to benefit from caching optimizations.
   * Otherwise, it delegates to doComputePreferredSize. Subclasses should
   * overload one of doComputeMinimumSize, doComputeMaximumSize, or
   * doComputePreferredSize to specialize the return value.
   * 
   * @see LayoutPart#computePreferredSize(boolean, int, int, int)
   */
  public final int computePreferredSize( boolean width,
                                         ...
                                         int preferredParallel )
  {
    ...
    if( preferredParallel == 0 ) {
      return Math.min( availableParallel,
                       computeMinimumSize( width, availablePerpendicular ) );
    } else if(    preferredParallel == INFINITE 
               && availableParallel == INFINITE )
    {
      return computeMaximumSize( width, availablePerpendicular );
    }
    // Optimization: if this subtree doesn't have any size preferences beyond
    // its minimum and maximum
    // size, simply return the preferred size
    if( !hasSizeFlag( width, RWT.FILL ) ) {
      return preferredParallel;
    }
    int result = doComputePreferredSize( width,
                                         availableParallel,
   ...
   * @param b indicates whether the caller wants the flags for computing widths
   *          (=true) or heights (=false)
   * @return a bitwise combiniation of flags with the same meaning as
   *         StackPresentation.getSizeFlags(boolean)
   */
  ...
  /**
   * Subclasses should overload this method instead of
   * computePreferredSize(boolean, int, int, int)
   * 
   * @see org.eclipse.ui.presentations.StackPresentation#computePreferredSize(boolean,
   *      int, int, int)
   * @since 3.1
   */
  protected int doComputePreferredSize( boolean width,
                                        int availableParallel,
                                        int availablePerpendicular,
                                        int preferredParallel )
  {
    int result = Math.min( availableParallel,
                           part.computePreferredSize( width,
                                                      availableParallel,
                                                      availablePerpendicular,
                                                      preferredParallel ) );
   ...
   * computePreferredSize(width, INFINITE, availablePerpendicular, 0). Returns a
   * cached value if possible or defers to doComputeMinimumSize otherwise.
   * Subclasses should overload doComputeMinimumSize if they want to specialize
   * the return value.
   * 
   * @param width true iff computing the minimum width, false iff computing the
   *          minimum height
   * @param availablePerpendicular available space (pixels) perpendicular to the
   *          dimension being computed. This is a height when computing a width,
   *          or a width when computing a height.
   * @see LayoutPart#computePreferredSize(boolean, int, int, int)
   */
  public final int computeMinimumSize( boolean width, 
                                       int availablePerpendicular )
  {
    assertValidSize( availablePerpendicular );
    // Optimization: if this subtree has no minimum size, then always return 0
    // as its
    // minimum size.
    if( !hasSizeFlag( width, RWT.MIN ) ) {
      return 0;
    }
    ...
    if( !hasSizeFlag( width, RWT.WRAP ) ) {
      availablePerpendicular = INFINITE;
    }
    if( width ) {
      // Check if we have a cached width measurement (we can only return a
      // cached
      ...
      // Check if we have a cached height measurement (we can only return a
      // cached
   ...
   * statistics for all LayoutTree instances.
   * 
   * @since 3.1
   */
  ...
  {
    assertValidSize( availablePerpendicular );
    // Optimization: if this subtree has no maximum size, then always return
    // INFINITE as its
    // maximum size.
    if( !hasSizeFlag( width, RWT.MAX ) ) {
      return INFINITE;
    }
    // If this subtree doesn't contain any wrapping controls (ie: they don't
    // care
  ...
  protected int doComputeMaximumSize( boolean width,
                                      int availablePerpendicular )
  {
    return doComputePreferredSize( width,
                                   INFINITE,
                                   availablePerpendicular,
                                   INFINITE );
  }
  ...
  public void flushNode() {
    // Clear cached sizes
    cachedMinimumWidthHint = RWT.DEFAULT;
    cachedMinimumWidth = RWT.DEFAULT;
    cachedMinimumHeightHint = RWT.DEFAULT;
    cachedMinimumHeight = RWT.DEFAULT;
    cachedMaximumWidthHint = RWT.DEFAULT;
    cachedMaximumWidth = RWT.DEFAULT;
    cachedMaximumHeightHint = RWT.DEFAULT;
    cachedMaximumHeight = RWT.DEFAULT;
    // Flags may have changed. Ensure that they are recomputed the next time
    // around
    sizeFlagsDirty = true;
    // The next setBounds call should trigger a layout even if set to the same
    // bounds since
   ...
   * out of synch with their cached information (for example, if a lot of
   * changes may have happened without calling flushCache after each change)
   * 
   * @since 3.1
   */
  ...
  /**
   * Inserts a new child on the tree. The child will be placed beside the
   * <code>relative</code> child. Returns the new root of the tree.
   */
  public LayoutTree insert( LayoutPart child,
                            boolean left,
                            LayoutPartSash sash,
                            LayoutPart relative )
  {
  ...
  /**
   * Find a child in the tree and remove it and its parent. The other child of
   * its parent is placed on the parent's parent. Returns the new root of the
   * tree.
   */
  ...
  /**
   * Sets the bounds of this node. If the bounds have changed or any children
   * have changed then the children will be recursively layed out. This
   * implementation filters out redundant calls and delegates to doSetBounds to
   * layout the children. Subclasses should overload doSetBounds to lay out
   * their children.
   * 
  ...
  /**
   * Creates RWT controls owned by the LayoutTree (ie: the sashes). Does not
   * affect the LayoutParts that are being arranged by the LayoutTree.
   * 
   * @param parent
   * @since 3.1
   */
   ...
   * hash code, the description should compare as equal iff the layouts are the
   * same. However, it should be user-readable in order to help debug failed
   * tests. Although these are english readable strings, they should not be
   * translated or equality tests will fail.
   * <p>
...
//  }
  ...
  /**
   * This is a shorthand method that checks if the tree contains the given size
   * flag. For example, hasSizeFlag(false, RWT.MIN) returns true iff the
   * receiver enforces a minimum height, or hasSizeFlag(true, RWT.WRAP) returns
   * true iff the receiver needs to know its height when computing its preferred
   * width.
   * 
   * @param vertical
   * @return
