...
//import org.eclipse.rap.rwt.widgets.ProgressBar;
import org.eclipse.rap.rwt.widgets.Sash;
//import org.eclipse.rap.rwt.widgets.Scale;
//import org.eclipse.rap.rwt.widgets.Slider;
import org.eclipse.rap.rwt.widgets.Text;
import org.eclipse.rap.rwt.widgets.ToolBar;
import org.eclipse.rap.rwt.widgets.Tree;

/**
 * Caches the preferred size of an RWT control
 * 
 * @since 3.0
 */
public class SizeCache {
    ...
    private Point getPreferredSize() {
        if (preferredSize == null) {
            preferredSize = computeSize(control, RWT.DEFAULT, RWT.DEFAULT);
        }
     ...
     *  
     * @param widthHint the known width of the control (pixels) or RWT.DEFAULT if unknown
     * @param heightHint the known height of the control (pixels) or RWT.DEFAULT if unknown
     ...
     * (this tries to compensate for RWT's hints, which aren't really the outer width of the
     * control).
     * 
     * @param control
     * @param widthHint
    ...
    private Point computeSize(Control control, int widthHint, int heightHint) {
        int adjustedWidthHint = widthHint == RWT.DEFAULT ? RWT.DEFAULT : Math
                .max(0, widthHint - widthAdjustment);
        int adjustedHeightHint = heightHint == RWT.DEFAULT ? RWT.DEFAULT : Math
    ...
    static boolean independentLengthAndWidth(Control control) {
        ...
        if (control instanceof Label || control instanceof Text) {
            return (control.getStyle() & RWT.WRAP) == 0;
        }
     ...
     * compensate for bug 46112. To be removed once RWT provides an "official"
     ...
     * safely return false for anything else. AFAIK, all RWT controls have this
