...
class LayoutPartSash extends LayoutPart {
  ...
  /**
   * Stores whether or not the sash is visible. (This is expected to have a
   * meaningful value even if the underlying control doesn't exist).
   */
...
//    Rectangle eventRect = new Rectangle( event.x,
//                                         event.y,
//                                         event.width,
//                                         event.height );
//    boolean vertical = ( style == RWT.VERTICAL );
...
//    LayoutTreeNode.ChildSizes sizes = node.computeChildSizes( nodeBounds.width,
//                                                              nodeBounds.height,
//                                                              left,
//                                                              right,
//                                                              nodeBounds.width );
//    eventRect.x = nodeBounds.x + sizes.left;
//    // If it's a horizontal sash, restore eventRect to its original coordinate
//    // system
//    if( !vertical ) {
//      Geometry.flipXY( eventRect );
//    }
...
//  }
  ...
  /**
   * Creates the control. As an optimization, creation of the control is
   * deferred if the control is invisible.
   */
  ...
  private void doCreateControl() {
    if( sash == null ) {
//      sash = new Sash( this.rootContainer.getParent(), style | RWT.SMOOTH );
      sash = new Sash( this.rootContainer.getParent(), style/* | RWT.SMOOTH*/ );
...
//      sash.setEnabled( enabled );
  ...
  /**
   * Makes the sash visible or invisible. Note: as an optimization, the actual
   * widget is destroyed when the sash is invisible.
   */
  ...
  boolean isHorizontal() {
    return ( ( style & RWT.HORIZONTAL ) == RWT.HORIZONTAL );
  }

  boolean isVertical() {
    return ( ( style & RWT.VERTICAL ) == RWT.VERTICAL );
  }

  void setPostLimit( final LayoutPartSash newPostLimit ) {
