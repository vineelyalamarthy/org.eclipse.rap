 ...
 ******************************************************************************/
package org.eclipse.rap.ui.internal.dnd;
...
public class DragUtil {
  ...
  /**
   * The location where all drags will end. If this is non-null, then all user
   * input is ignored in drag/drop. If null, we use user input to determine
   * where objects should be dropped.
   */
//  private static TestDropLocation forcedDropTarget = null;
  ...
  /**
   * Sets the drop target for the given control. It is possible to add one or
   * more targets for a "null" control. This becomes a default target that is
   * used if no other targets are found (for example, when dragging objects off
...
//  }
  ...
  /**
   * Return the list of 'IDragOverListener' elements associated with the given
   * control. If there's a 'global' listener then always return it.
   * 
   * @param control
   * @return
   */
...
//  }
   ...
   * display coordinates. Note that all 'Shell' controls are expected to be 'top
   * level' so DO NOT do the origin offset for them.
   * 
   * @param draggedItem
   * @param boundsControl
   * @return
...
//  {
//    IDropTarget target = dragToTarget( draggedItem,
//                                       sourceBounds,
//                                       initialLocation,
//                                       allowSnapping );
//    if( target == null ) {
...
//  }
   ...
   */
//  public static boolean dragTo( Display display,
//                                Object draggedItem,
//                                Point finalLocation,
//                                Rectangle dragRectangle )
//  {
//    Control currentControl = SwtUtil.findControl( display, finalLocation );
//    IDropTarget target = getDropTarget( currentControl,
//                                        draggedItem,
//                                        finalLocation,
//                                        dragRectangle );
//    if( target == null ) {
...
//  }
   ...
   * Intended for use by test suites. If this method is called, then all
   * subsequent calls to performDrag will terminate immediately and behave as
   * though the object were dragged to the given location. Calling this method
   * with null cancels this behavior and causes performDrag to behave normally.
   * 
...
//  }
   ...
   * coordinates. Due to a quirk in the Tracker class, changing the tracking
   * rectangle when using the keyboard will also cause the mouse cursor to move.
   * Since "snapping" causes the tracking rectangle to change based on the
   * position of the mouse cursor, it is impossible to do drag-and-drop with the
   * keyboard when snapping is enabled.
   * 
   * @param draggedItem object being dragged
   ...
   * @param allowSnapping true iff the rectangle should snap to the drop
   *          location. This must be false if the user might be doing
...
//      Control currentControl = SwtUtil.findControl( forcedDropTarget.getShells(),
//                                                    location );
...
//            }
//            // If snapping then reset the tracker's rectangle based on the
//            // current drop target
//            if( allowSnapping ) {
//              if( snapTarget == null ) {
...
//                                            sourceBounds.height );
//              }
//              // Try to prevent flicker: don't change the rectangles if they're
//              // already in
//              // the right location
...
//      }
//    } );
//    // Setup...when the drag starts we might already be over a valid target,
//    // check this...
...
//    if( startRect != null ) {
//      tracker.setRectangles( new Rectangle[]{
//        Geometry.copy( startRect )
//      } );
//    }
//    // Tracking Loop...tracking is preformed on the 'SWT.Move' listener
//    // registered
//    // against the tracker.
//    // HACK:
...
//    Control targetControl = display.getCursorControl();
//    dropTarget = getDropTarget( targetControl,
//                                draggedItem,
//                                finalLocation,
//                                tracker.getRectangles()[ 0 ] );
//    // Cleanup...
//    tracker.dispose();
//    // if we're going to perform a 'drop' then delay the issuing of the
//    // 'finished'
...
//  }
  ...
  /**
   * Given a list of IDragOverListeners and a description of what is being
   * dragged, it returns a IDropTarget for the current drop.
   * 
   * @param toSearch
...
//    return getDropTarget( defaultTargets,
//                          toSearch,
//                          draggedObject,
//                          position,
//                          dragRectangle );
