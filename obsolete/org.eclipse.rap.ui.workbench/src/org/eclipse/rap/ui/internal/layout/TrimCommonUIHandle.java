...
//import org.eclipse.rap.jface.action.ContributionItem;
...
import org.eclipse.rap.rwt.widgets.CoolItem;
...
//			}
     ...
     * @param curSide  the RWT side that the trim is currently on
     */
    public TrimCommonUIHandle(TrimLayout layout, IWindowTrim trim, int curSide) {
    	super(trim.getControl().getParent(), RWT.NONE);
...
//    	this.radioVal.set(curSide);
    	...
    	orientation = (curSide == RWT.LEFT || curSide == RWT.RIGHT) ? RWT.VERTICAL  : RWT.HORIZONTAL;
...
//        cb.addListener(RWT.MenuDetect, menuListener);
...
//    	LayoutUtil.resize(trim.getControl());
	...
	private void createWindowTrimProxy() {
		...
		WindowTrimProxy proxy = new WindowTrimProxy(this, "NONE", "NONE", //$NON-NLS-1$ //$NON-NLS-2$
				RWT.TOP | RWT.BOTTOM | RWT.LEFT | RWT.RIGHT, false);
	...
	private int getHandleSize() {
		...
		if (orientation == RWT.HORIZONTAL && horizontalHandleSize != -1) {
			return horizontalHandleSize;
		}
				
		if (orientation == RWT.VERTICAL && verticalHandleSize != -1) {
...
//		
//		Label ctrl = new Label (bar, RWT.PUSH);
//		ctrl.setText ("Button 1"); //$NON-NLS-1$
//	    Point size = ctrl.computeSize (RWT.DEFAULT, RWT.DEFAULT);
//		
...
//		cb = new CoolBar(this, orientation | RWT.FLAT);
//		cb.setLocation(0,0);
//		ci = new CoolItem(cb, RWT.FLAT);
//		
//		// Create a composite in order to get the handles to appear
//		Composite comp = new Composite(cb, RWT.NONE);
//		ci.setControl(comp);
	}
...
//    	Cursor dragCursor = toDrag.getDisplay().getSystemCursor(RWT.CURSOR_SIZEALL);
...
//    				if (trim.isCloseable()) {
//	    				MenuItem closeItem = new MenuItem(menu, RWT.PUSH, index++);
...
//    				dockCascade = new MenuItem(menu, RWT.CASCADE, index++);
//    				{
...
//    					
//						radioButtons.addMenuItem(WorkbenchMessages.TrimCommon_Top, new Integer(RWT.TOP));
//						radioButtons.addMenuItem(WorkbenchMessages.TrimCommon_Bottom, new Integer(RWT.BOTTOM));
//						radioButtons.addMenuItem(WorkbenchMessages.TrimCommon_Left, new Integer(RWT.LEFT));
//						radioButtons.addMenuItem(WorkbenchMessages.TrimCommon_Right, new Integer(RWT.RIGHT));
//    					
...
////    				showCascade = new MenuItem(menu, RWT.CASCADE, index++);
////    				{
...
////							MenuItem item = new MenuItem(showMenu, RWT.CHECK);
...
//        removeListener(RWT.MenuDetect, menuListener);
