...
import org.eclipse.rap.ui.internal.presentations.util.AbstractTabFolder;
import org.eclipse.rap.ui.internal.presentations.util.AbstractTabItem;
import org.eclipse.rap.ui.internal.presentations.util.EnhancedFillLayout;
import org.eclipse.rap.ui.internal.presentations.util.PartInfo;

/**
 * Implements the AbstractTabFolder interface, however this object only displays
 * the content of the currently selected part. There are no tabs, no title, no
 * toolbar, etc. There is no means to select a different part, unless it is done
 * programmatically.
 * 
 * @since 3.1
 */
...
//      } );
   ...
   * (non-Javadoc)
   * 
   * @see org.eclipse.ui.internal.presentations.util.AbstractTabFolder#computeSize(int,
   *      int)
   */
  public Point computeSize( int widthHint, int heightHint ) {
    if( childControl != null ) {
      // Fix for bug 85899 [Perspectives] [RCP]Standalone view does not divide
      // space in
      // proper ratio with reference when added to IPageLayout with showTitle
      // parameter false
      // If the childControl is a content proxy (a Composite with no children --
      // see PresentablePartFolder constructor),
      // then computeSize returns 64x64, which is inappropriate.
      // Note that this happens even if the Composite has a Layout that returns
      // 0@0.
      // Instead, use the sizes of the margins. This is equivalent to calling
      // control.computeSize(...)
      ...
      if( childControl instanceof Composite ) {
        ...
        if( composite.getChildren().length == 0 ) {
          ...
          int w = widthHint == RWT.DEFAULT
                                          ? layout.xmargin * 2
                                          : widthHint;
          int h = heightHint == RWT.DEFAULT
                                           ? layout.ymargin * 2
                                           : heightHint;
...
//      return childControl.computeSize( widthHint, heightHint );
      return childControl.computeSize( widthHint, heightHint, true );
    }
